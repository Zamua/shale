...
// NOTE: there is NO operator-facing Reshard handler. Resharding is DECLARATIVE
// (v0.8): the desired unit count rides the deploy config (--unit-count) and the
// cluster's deterministic coordinator reshards itself toward it via the
// ReshardControl barrier above. The prior Server.Reshard handler + its proto
// messages + the `shale reshard` CLI were deleted because the trigger is now
// config (k8s RBAC / CI), not a network RPC that would need its own authz. See
// docs/SPEC.md "Declarative resharding".
