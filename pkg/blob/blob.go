...
// FinalKey / StagingKey helpers in this package); the adapter does not prepend
// any prefix of its own beyond an optional bucket-internal KeyPrefix.
type Store interface {
	...
	// List yields every object key under prefix, for the future same-shard
	// orphan-bytes sweep (phase 2). It is an iter.Seq2 so the caller can stop
	// early and so a listing error surfaces per-item rather than buffering the
	// whole listing; once a non-nil error is yielded the sequence ends.
