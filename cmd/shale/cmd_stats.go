...
//
// It reports only fields the server actually measures. Latency percentiles
// are NOT among them: the Stats RPC never carried a histogram, so the old
// latency_ms_p50 / latency_ms_p99 columns printed a constant 0 that read as
// telemetry. Real percentiles come from `shale bench`, which times the ops it
// issues.
