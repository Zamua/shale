...
The overlap machine above makes scale-UP (a JOIN) mount-time-independent: a joining node ACQUIREs while the existing owner keeps `Draining`-serving, and the position is never unserved. Scale-DOWN (a deliberate node REMOVAL) is the SYMMETRIC case, and it has a gap the overlap machine does NOT yet close, because the gap is in the SHUTDOWN PATH, not the reconcile path. When a node is removed gracefully (a SIGTERM from the operator / orchestrator), the surviving nodes re-own its positions and start `Acquiring` them (forwarding to the leaving node as the predecessor) exactly as for any membership change - from the LEAVING node's own view, every position it owns becomes `Draining` (a surviving node took its exact position, a single-hop move), so it KEEPS SERVING during the survivors' mounts. The overlap drain is therefore already the right machinery for a graceful leave; the only missing piece is that the leaving node's `Close()` does not WAIT for that drain to finish.

**The gap today (the availability bug this subsection closes).** On SIGTERM the run loop calls `Cluster.Close()`, which broadcasts the memberlist leave (so peers re-own the units and begin forwarding to the leaving node) but then IMMEDIATELY tears down the reconcile loops + peer clients + the mounted units, WITHOUT waiting for the survivors' (slow) `Acquiring` to complete. So every position the leaving node was serving goes UNSERVED from the instant `Close()` closes its mounts until the survivors' mounts flip - exactly the mount-time gap Phase 2e removed for scale-up, reopened for scale-down. The serving + `drainCheck` + forward path that would have held availability are torn down before they could.

**The fix: drain BEFORE teardown, bounded by a grace timeout.** A deliberately-leaving node broadcasts the memberlist leave, then keeps serving its now-`Draining` units (its own routed ops AND the survivors' position-addressed forwards, exactly the overlap forward path) until every one of them has been handed off to a SERVING successor - i.e. until `drainCheck` has released them all (each released on observing the successor's serving marker at an epoch strictly above its own open epoch, the SAME release rule as a normal overlap drain). Only THEN does the node proceed with the existing `Close()` teardown. During the drain wait the reconcile loop + serving + `drainCheck` + the forward path MUST stay alive (the teardown does not begin until the drain returns). The wait is bounded by a grace timeout: if a successor is stuck and the drain does not complete within the budget, the node closes anyway, accepting the residual gap for the unhanded-off positions (no worse than today's behavior for those positions).

**Two memberlist verbs, split (Leave vs Shutdown).** memberlist exposes `Leave()` (broadcast the graceful departure so peers record a clean leave and re-own the units) DISTINCT from `Shutdown()` (tear down the local transport). The membership wrapper's `Close()` today does BOTH at once, which is why the leaving node stops being reachable the moment it begins to close. Phase 2e splits them: a new `Leave()` broadcasts the departure WITHOUT shutting the transport down (so the node keeps serving gRPC and keeps being FORWARDED-TO throughout the drain), and `Shutdown()`/`Close()` of the transport stays where it is, in the existing teardown after the drain returns.

**The drain entry point.** A new `Cluster` method `DrainForLeave(ctx)` (equivalently `GracefulLeave(timeout)`): broadcast the graceful leave via the new membership `Leave()` (so peers re-own this node's units and start forwarding to it), then BLOCK until the `handoffPhase` map holds no `Draining` entry owned by this node (every position `drainCheck` released, `mountMap` carries no position this node still owns) OR `ctx` is cancelled / the timeout fires. The reconcile loop, the serving path, `drainCheck`, and the forward path all stay running during this wait. Then return. The shutdown path calls `DrainForLeave` at the TOP of `Close()` - gated on config, BEFORE any teardown, while the loops are still running - and then proceeds with the existing teardown unchanged. Putting the drain at the top of `Close()` keeps it self-contained: the SIGTERM handler in the run loop just calls `Close()` as it does today, so no run-loop change is required.
