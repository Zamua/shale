...
#### Failure modes
  ...
  - **Replica mid-handoff (rebalance)**: the receiving node returns `codes.ResourceExhausted` (the migration-guard sentinel), distinct from `Unavailable`. The fanout classifies it as transient (skip this replica, wait for another) and does NOT count it against the failure budget. Without that distinction, a single mid-handoff replica during normal v0.3 rebalance would force every otherwise-quorum write to wait for the migration to complete. (The v0.8 multi-backend acquiring-window refusal `errUnitAcquiring` is the SAME class: it is sentinel-tagged so `isTransientReplicaErr` skips it too even though it travels client-facing as `codes.Unavailable`, and the cross-node replica leg re-codes it to `codes.ResourceExhausted`; see "v0.8 Phase 2d".)
...
**The signal: readiness (authoritative), the durable epoch (a liveness hint that must be re-verified).** There is NO new central controller and NO new coordination protocol for correctness. Each node runs `reconcileReplicaUnits` independently off the existing debounced settle timer, driven by the same membership topology events; the ring is the shared truth and both nodes independently conclude the position moves. Two cross-node signals, with DISTINCT roles:

  1. AUTHORITATIVE (the release trigger): the `ReplicaHandoffReady(ru, E)` RPC the new owner sends ONLY AFTER its mount flip (so it proves a live owner is serving the position). The old owner releases on it. Reuses the existing `clientFor` / rpc-server plumbing.
  2. LIVENESS HINT (NOT a release trigger on its own): the DURABLE per-replica manifest writer-epoch, readable WITHOUT opening via `DurableEpochReplica(ru)` (the slate `Backing` already exposes it; Phase 2e lifts it onto the `ReplicaBackendFactory` interface). Seeing it cross the old owner's open epoch proves SOMEONE fenced, NOT that a live writer is serving (a new owner that fences and then crashes mid-mount advances the epoch without ever serving). On seeing the epoch cross, the old owner actively RE-VERIFIES via a readiness probe to the new replica that a live owner is serving the position, and releases ONLY on a positive confirmation. This is the crash-proof fallback when the RPC is lost; a BARE epoch advance NEVER releases. The release-check is armed as a bounded periodic re-check on the existing settle / self-heal cadence and woken early by the RPC. **The re-verify is a POINT-IN-TIME liveness hint, not a lease/latch:** if NEW crashes in the gap between the old owner's positive probe and the completion of its release (the old owner does not re-probe inside the lock - I/O under the lock is forbidden), the old owner releases and the position is unserved until the NEXT RECONCILE reassigns it, with NO acked-write loss (durable-before-ack). So the re-verify makes the old owner safe against a NEW that crashed BEFORE becoming Ready (crash case (a)), NOT against a crash in the release gap; the gap's recovery is the benign next reconcile (see guarantee 3).
...
**THE FIVE CORRECTNESS GUARANTEES (argued explicitly).**
  ...
  3. CRASH SAFETY (NOT unconditionally "fully available"; see the R/W matrix in the design doc). (a) NEW crashes mid-acquire (`Acquiring`, before the mount flip): it never sent `ReplicaHandoffReady`; a bare durable-epoch advance from its pre-crash fence does NOT release the old owner (the readiness re-verify probe to NEW fails), so the old owner stays `Draining` and KEEPS SERVING. The only effect is that ops in-flight to the crashed new owner fail and retry; the next reconcile reassigns the position (possibly back to OLD, which drops `Draining` and returns to Owned). This is the crash the release-on-Ready fix protects: under a bare-epoch release NEW's pre-crash fence would have stranded the position. (b) OLD crashes while `Draining` (before NEW is Ready): the predecessor the new owner forwards to is gone, so the new owner falls through to `errUnitAcquiring` for the position, degrading to the Option-A window for THIS one position only, bounded by the new owner's mount, and only when the churn also kills the old owner mid-flight. Availability during this window depends on the R/W config: at R>=3/W=majority or R=2/W=1 the other replicas cover W; at R=2/W=2 (write-all) the position is write-unavailable until NEW mounts (only one live replica). Every acked write is already durable (durable-before-ack), so NEW sees them all once it mounts; no acked write is lost. (c) NEW crashes AFTER Ready but before OLD released: if `ReplicaHandoffReady` reached OLD, OLD already released and the next reconcile reassigns the position; if it did not, the epoch-cross hint fires but the readiness re-verify probe to NEW now FAILS, so OLD does NOT release on the bare epoch and stays `Draining`-serving until the next reconcile reassigns it. Either way no acked write is lost (all durable), and any stale OLD mount is torn down by `evictStaleMount` on its next fenced write. RESIDUAL (the re-verify is point-in-time, not a latch): if NEW crashes in the gap between OLD's positive probe and the completion of OLD's release (OLD does not re-probe inside the lock), OLD releases anyway and the position is UNSERVED until the next reconcile reassigns it. This is benign (no acked-write loss; durable-before-ack), and the recovery is the same next-reconcile path. The re-verify closes the bare-epoch hole (it makes OLD safe against a NEW that crashed BEFORE Ready, case (a)); it does NOT make case (c) fully crash-safe against a crash-in-the-release-gap, and the doc does not claim it does.
...
**Resolved review findings.** Two adversarial reviews (`docs/design/overlap-handoff-review.md` first pass and `docs/design/overlap-handoff-rereview.md` re-review of the forwarding design, both kept as history) found holes the behavior above now closes:
  ...
  - P1-1 (release on a bare durable-epoch advance): closed by making "NEW OWNER IS READY" the authoritative release trigger (`ReplicaHandoffReady`, sent only after the mount flip); the durable epoch is a liveness HINT that triggers a readiness RE-VERIFY before releasing, never a bare-epoch release. This also fixes the crash-case-1 regression.
...
The re-review found NEW holes the FORWARDING approach itself introduced; these are closed by the five operator decisions folded into the behavior above:
  ...
  - NEW-P1-3 (the readiness re-verify is point-in-time, so a NEW crash in the probe-to-release gap lets OLD release with nobody serving): closed as an HONESTY fix - stated that the re-verify is a point-in-time liveness hint; the gap leaves the position unserved until the next reconcile with NO acked-write loss, and the doc no longer claims case-3 is fully crash-safe.
