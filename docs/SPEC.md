...
**Why ring-based and not Jump Consistent Hash:** Jump (Lamping-Veach 2014) is zero-state and faster but requires bucket numbers 0..N-1 - removing an arbitrary middle bucket requires re-numbering, which doesn't compose with our dynamic membership, where any node can leave anywhere. Ring-based fits dynamic topology naturally at the cost of a few KB of in-memory state per node + an O(log N) binary search per lookup. Both negligible at our scales.
...
**The gap this subsection closes, AND why the obvious fix is self-contradictory.** On SIGTERM the run loop calls `Cluster.Close()`. The naive fix - "ANNOUNCE the departure but keep serving, so survivors re-own the positions while the leaving node still answers" - DOES NOT WORK, and that self-contradiction was the root cause of the measured ~58% in-process / ~70% staging drain availability. The leaving node is being asked to be BOTH gone (so survivors take over its ownership positions) AND present (so it can keep serving during the drain), and a departure announcement cannot represent that. The two answers are exclusive: announce the departure and the node loses the positions it is still serving; stay present and no survivor ever starts the handoff. (The measured instance was the removed gossip adapter, where `memberlist.Leave()` while the transport stayed up made survivors drop the leaver and then RE-ADD it on the next alive gossip, so the handoff never started. The lesson is adapter-independent: departure and presence do not compose, so the drain must not be started by departing.)

**The fix: a distinct DRAINING node-state - REACHABLE, a CURRENT OWNER, and the source of the PENDING split.** The node needs a state a bare departure cannot express: it stays a full, live, addressable member AND a current owner of its positions (so routing keeps dual-writing it and it keeps serving), while its `Draining` role makes every node compute the PENDING set (ring-minus-this-node) and route the UNION. This is advertised as a per-member ROLE through the coordination port, NOT by leaving the cluster. The sequence:

  - The leaving node SETS ITSELF DRAINING by publishing `coord.RoleDraining` through the coordinator (`SetRole`); under the CAS adapter that is a CompareAndSet on this node's row in the membership document, alongside the gRPC dial address the row already carries. The node stays LIVE and a current owner - it does NOT depart here, and it is NOT removed from the ownership ring.
  - Every node (including the draining node itself) reads each peer's role set from the coordinator's view and learns the `Draining` role. `routedReplicasForKey` consumes it: for a position the leaver currently owns, CURRENT (ring including the leaver) != PENDING (ring excluding the leaver), so the position is IN TRANSITION and the routed set becomes UNION(current, pending). The leaver stays in CURRENT (mounted, serving), and the successor that takes the leaver's exact position is in PENDING.
...
**THE BUDGET IS A LIVENESS ESCAPE HATCH, AND IT TRADES AVAILABILITY.** `DrainForLeave` ends on hand-off OR on `GracefulLeaveDrainTimeout`, whichever comes first, and the timeout case departs ANYWAY. That is deliberate: a leave must not hang forever on a successor whose mount is stuck. The cost is exact and worth stating rather than discovering: every position still awaiting a successor at that moment is UNSERVED from teardown until its successor finishes mounting, because the leaver is removed from the ring and its mounts go with the process, while the successor is by definition not yet serving. A routed read or write for such a position finds NO holder anywhere in the routed union and gets the retryable acquiring refusal - not a lost write (the durable copy is in shared storage and the successor opens it), but a real availability window whose length is the successor's remaining mount time.

Because that window is indistinguishable downstream from a routing bug, **the departing node NAMES it**: on budget expiry the leaver logs how many positions it abandoned and which, so the gap is explicable from the leaver's own output instead of inferred from refusals observed elsewhere minutes later. A leave that hands off cleanly logs nothing. (`positionsAwaitingSuccessor` is the evidence half of the completion gate; the loud line is pinned by `TestLeave_BudgetExpiry_NamesTheUnservedPositions`.)
...
**The fix: learn the live generation at Open, before mounting - deferring while a reshard is in flight.** Two paths, by configuration:
  ...
  - **No ConditionalStore (the peer-RPC path):** the joiner queries each visible peer's `GenState` RPC for `{gen, count, reshard_in_flight}` BEFORE it derives or mounts any unit, and commits the answer. A seed reporting `reshard_in_flight` (its `nextCount` is set) is NOT an answer: the joiner defers and re-sweeps (within `GenLearnBudget`) until a seed is steady, then seeds `genState` from the stable value. Only if every seed stays unreachable / rejecting / in-flight for the whole budget does `Open` FAIL rather than fall back to gen 0 (fail closed, supervised restart). The patience mechanics (re-sweep with backoff, the cold-starting-seed rationale, `genQueryTimeout`) are unchanged.
...
**Advertise the declared count; retarget only on unanimity + steadiness (flap-proof).** Each node advertises its standing declared unit count through the coordinator (`coord.Params.DeclaredUnitCount`, carried per-member in the membership document). On every R>1 reconcile tick, `observeDeclaredReshardTarget` runs alongside `observeReshard` and retargets the arbiter to a declared count `D` ONLY when ALL of:
  ...
  - EVERY live member (including self) advertises a KNOWN declared count and they are ALL EQUAL to the same `D` (UNANIMITY); a member with an unknown/absent declared count - an older image that does not advertise it - breaks unanimity and defers the retarget (fail-safe); and
...
#### Decided against: an operator Reshard RPC and a real-process chaos harness

Two subsections here once specified future work that will NOT be built, and
are removed rather than left to read as a plan: an operator-facing `Reshard`
RPC on `ShaleNode` (resharding stays a library call, not a wire operation),
and a `chaosreal` harness driving separate `shaled-slate` processes over real
gRPC and a real bucket. The harness once existed (`tests/chaos/adapter_real.go`,
build tag `chaosreal`) and was deleted; nothing in the tree provides
cross-process chaos validation, and no comment should claim otherwise. The
in-process chaos soak (`tests/chaos`) remains the model-level validation.
#### The deployable run path (exact wiring contract)

The config-surface section above states WHAT the deploy gap closes; this section pins the EXACT wiring so the implementation builds to a single contract. It does NOT change any `pkg/cluster` multi-backend LOGIC (the Phase 2/2b/3/4 + reshard machinery is unchanged): it only exposes that already-validated machinery to a deployable binary. The single-backend path (the `memory` / `pebble` / single-`slate` binaries) stays byte-for-byte except that `ReplicationFactor` is now threaded through (today it is dropped, pinning single-backend to the cluster default R=1; see below). The carried-over invariant is unchanged: NO ACKED WRITE IS LOST. This wiring acks a write exactly when `cluster.Cluster` does; it adds no write path.
...
`shaled` (memory) and `shaled-pebble` are single-node demonstration and test
daemons. They wire a single `Backend` and no coordinator, and neither exposes a
flag that would make it join a cluster: a local LSM has no shared durable copy
for a peer to take over, so multi-node needs a coordination adapter AND a
`BackendFactory`.

`shaled-slate --multi-backend` is the deployable multi-node binary. It
constructs the CAS coordinator over the same bucket the units live in
(`--coord-key` / `SHALE_COORD_KEY` names the membership document, defaulting to
`cas.DefaultKey`) and passes a `BackendFactory`, so every node launches from
the SAME config: no bind address, no seed list, nothing per-node beyond
`--node-id`. There is no transport-level cluster wiring left on the standard
flag surface (`BindStdFlags` registers `--node-id`, `--grpc-addr`,
`--replication-factor`, `--unit-count`, `--graceful-leave-drain` and
`--write-timeout`); coordination is an OPERATOR-CONSTRUCTED object, not a
flag. `Config.Backend` together with a `Coordinator` is refused at `Open` with
a message naming the multi-backend requirement, so a single-backend binary
cannot be talked into multi-node mode.

## The coordination port (pkg/coord)

The cluster does not discover, probe or track peers directly: it asks a
`coord.Coordinator` port. `pkg/coord/cas` is the ONE SHIPPED adapter (see "The
CAS coordinator" below). Whatever a given adapter does inside - a mesh, a
polled document, a control plane - is invisible through the port, and nothing
above the port names it. `pkg/coord` itself imports only `errors` and
`pkg/storageunit`.
...
overlap warm-up. A different coordinator replaces how the answers are produced,
never what the storage layer does with them - which is what makes the port a
real seam and not a rename.
...
**CONSTRUCTION: the coordinator is operator-supplied.** A multi-node consumer
builds the adapter and hands it to `Open`; a single-node consumer passes
nothing (`Coordinator: nil` IS single-node):

```go
// multi-node
cluster.Open(cluster.Config{ /* ... */ Coordinator: cas.New(cas.Config{
    Store: condStore, // the same storageunit.ConditionalStore the units use
    Key:   coordKey,  // empty selects cas.DefaultKey
})})

// single-node: no coordinator at all
cluster.Open(cluster.Config{ /* ... */ })
```

The cluster's own transport fields went with this seam. `Config.BindAddr` and
`Config.Seeds` were REMOVED in v0.15.0 (they were gossip transport details, and
the port exists so the cluster does not name them); `pkg/coord/gossip` itself
was removed after v0.18.1, so there is no adapter left that takes a bind
...
membership document in a `storageunit.ConditionalStore` (`PutIfAbsent` /
...
`MemConditionalStore`). It therefore REQUIRES a conditional store, which is
the deployment shape shale already targets: the store the data lives in
becomes the one truth channel, with no mesh, no seed list and no extra port to
operate. Nothing above the port depends on that choice: serving markers, the
drain protocol and `DrainForLeave`, overlap warm-up, boot-defer / generation
learning, the settle debounce are all built on the port's ANSWERS, so a fork
that implements the port for a different mechanism inherits them unchanged.
...
out, so the dual-basis window the removed gossip adapter carried (ring trails
the snapshot, heals on reconcile) cannot arise here. `PlacementMembers` stays
on the port because an adapter that mirrors two sources into each other CAN
deviate and the guards that read it must hold there too; under CAS it simply
never deviates.

**Placement is the ring's math, not the adapter's.** Same `pkg/ring`, same
`GenUnitBytes` hash input, same bounded-load construction. `Placement.Exclude`
is honored by genuinely rebuilding a reduced ring, meeting the exactness
contract (the placement that WILL HOLD after departure, never
filter-after-locate). Placement is a pure function of the live member set, so
any adapter that reports the same members answers Locate identically.
...
first row, atomic with the node's first appearance, closing the
member-before-warming race the port's `InitialRoles` clause names (and a
founder still drops a requested `RoleJoining`, per the port). `SoloStart`
collapses to store reachability: the conditional write makes founded-vs-joined
a definitive answer rather than a could-not-reach-peers guess, so there is no
tentative-then-correct dance and no silent-fork risk for the flag to guard. An unreachable STORE fails Start - the store is
...
contract; consumers re-read and reconcile on a cadence, exactly as the port
requires.
...
graceful-leave-vs-reaping split SWIM makes with Leave vs suspicion.
...
which the port allows. (One document and one truth channel make this adapter
the natural future home, but moving the arbiter is a separate change with its
own migration story.)

**There is nothing to choose between.** CAS is the only adapter shale ships,
and its one requirement - a `ConditionalStore` - is met by every multi-node
deployment shale targets, because the arbiter and the bootstrap marker already
need it. A deployment on a backend with no conditional writes is single-node.

The SWIM/gossip adapter (`pkg/coord/gossip` over hashicorp/memberlist, with
its `pkg/membership` wrapper) was the port's other adapter and was removed
after v0.18.1; `docs/design/coordinator-migration.md` holds the live-cluster
switchover choreography that moved deployments off it. What its removal costs
is stated above under the tradeoff: detection is `K * pollInterval` rather
than SWIM's probe-and-suspicion seconds, and the document is a serialization
point whose write rate grows with N.

**One contract, any adapter.** The port-contract tests are a shared harness,
`internal/coordcontract`: `RunContract(t, adapter)` pins Populated tracking
View, TransitionSets matching View's roles (nil-map steady state, retraction
back to nil, caller-owned maps), the PlacementMembers basis agreement, Locate
determinism across instances, primary-stable-across-N, clamping and degenerate
n, exclusion as a genuinely reduced placement, and exclude-everyone-returns-nil.
The CAS adapter runs the harness and keeps its own suite on top (lease expiry,
document GC, bootstrap races). A fork's adapter runs the same harness, which
is what makes the port a supported extension point rather than a leftover
abstraction.
...
- [ ] **v0.2** - multi-node + memberlist + hash ring + gRPC forwarding. Static topology (no rebalance). `shale topology` now shows real membership + ring. (Superseded: membership moved behind the coordination port in v0.15.0, and the memberlist adapter left the tree after v0.18.1 - see "The coordination port".)
...
- [~] **v0.8** (in progress) - per-shard lease-handoff storage: copy-free rebalance + online doubling resharding. Replaces the per-node single-database model (one slatedb per node, rebalance copies keys) with a FIXED power-of-two count N of self-contained storage-unit databases whose OWNERSHIP is a lease (the ring is re-keyed onto unit ids; handoff is close-on-old-owner / open-at-epoch+1-on-new-owner, zero bytes copied). N is bounded by per-engine memtable memory; growth by doubling makes resharding an online per-unit bisect with no global re-partition. Full design in the "Per-shard lease-handoff storage" section above. Sub-tasks:
  ...
  - [x] Generation propagation to a joining node: a multi-backend joiner (one whose bootstrap reports `BootstrapJoined`) learns the cluster's live `{generation, unit-count}` by a synchronous peer RPC (`GenState`) to a visible peer BEFORE it derives or mounts any unit, then seeds its `genState` from the answer - so it never routes / owns / serves a key at gen 0 after the cluster has resharded. Fixes the reshard-then-add-node acked-write-loss path (a gen-0 joiner orphans keys: `forwarding loop refused`). The query is PATIENT - it re-sweeps visible seeds with backoff for a bounded budget (`GenLearnBudget`, default 180s) so a seed that is still cold-starting (port bound, gRPC not yet serving until its own mount finishes) is WAITED FOR instead of crash-looping the joiner; fails closed only if every seed stays unreachable for the whole budget (Open fails, no gen-0 fallback). The founder + single-node + legacy paths keep the gen-0 default. See "Generation propagation to a joining node" above.
...
- **Olric** - the same embedded-node + consistent-hash pattern (over memberlist). In-memory only; we generalize to durable backends.
- **Vasto** - sharded RocksDB with jump consistent hash + single-master topology. Dormant; architecture is sound and we borrow heavily. We diverge by using ring-based hashing and by putting membership behind a coordination port.
