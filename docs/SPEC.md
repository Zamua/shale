...
### v0.9 Decentralized reshard (declarative split/merge, R>1) - IN DEVELOPMENT
...
- **Per-unit flip + finalize.** Every node polls the durable cut-over markers and, on observing one, sets local `cutOver[K]` (routing + the ack bar flip to the children) - the single cluster-ordered cut point, observed identically by every node, replacing the freeze's global ordering with a per-unit durable one. Once EVERY old unit has cut over, a final clean copy of every owned parent (the strict no-loss backstop) precedes advancing the live generation and retiring the obsolete parent mounts.

**Why no acked write is lost.** The ack bar is over the authoritative generation throughout; the cut-over marker is published only after every replica slot is caught up (the children provably hold everything the parents held when the marker appeared); after the marker, writes ack over the children, and a parent is retired only after a final clean copy confirms the children hold everything it holds. There is no instant when the only durable home of an acked write is a unit being closed, and the single cluster-ordered cut point is the durable marker.
...
**Scope (built).** Online SPLIT (`N -> 2N`) under stable membership. **Not yet built (subsequent phases, this section grows as they land):** the MERGE (`2N -> N`) direction; joiner-mid-reshard wire fields; `Transact` spanning a mid-split unit; orphan-child GC + a per-split abort deadline; backgrounding the copy for slow object stores (today it is synchronous under the reconcile lock - correct, and proven by the in-memory gate); composition with a concurrent membership change on the same unit.
