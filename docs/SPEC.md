...
- **Two-source copy + dual-parent caught-up.** Each parent slot is copied into the survivor by forwarding every key to the survivor's replicas APPLY-IF-NEWER with a WRITE-QUORUM ack per key (the cross-node merge copy). Because two parents feed one survivor, the cut-over markers for BOTH parents are published together, gated on BOTH parents' slots all being caught up - so the survivor provably holds both sources before either parent flips.
