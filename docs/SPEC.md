...
Phase 4 lets an operator GROW capacity by DOUBLING the unit count: N -> 2N at a new cluster generation. Each old unit K bisects into EXACTLY new units K and K+N by one additional hash bit (the math is built: `storageunit.ChildUnit` / `ChildUnits` / `UnitCount.Double` / `UnitForHash`). The bisect is ONLINE and per-unit: the old unit keeps serving while its data is copied, then an atomic per-unit cut-over flips routing. Built to a NO-ACKED-WRITE-LOST invariant. Multi-backend mode ONLY; the legacy per-node path is untouched. Assumes STABLE membership for the duration of a reshard.

- **Generation-qualified unit identity (the central change).** Today a unit maps to storage by UnitID alone (`Backing` keys `stores`/`epochs` by bare `UnitID`; `unitIDBytes` feeds the ring a bare id). That COLLIDES across a doubling: gen-g unit K (count N) and gen-(g+1) unit K (count 2N) would share one database. So a unit's STORAGE IDENTITY gains the generation: the storage key / object-store prefix is `(generation, UnitID)`, making gen-g unit K and gen-(g+1) unit K distinct databases that coexist during the bisect. Thread it through `BackendFactory` (OpenUnit/CloseUnit/CurrentEpoch/OpenUnits gain a generation, or a wrapping `(gen, UnitID)` identity type), the cluster `mountMap` (keyed by generation-qualified id), the ring routing (`unitIDBytes` includes the generation so the gen-g and gen-(g+1) id of the same K hash to potentially different owners), and the test factories.
...
- **The online per-unit bisect.** An explicit operator trigger (a reshard coordinator, e.g. `cluster.Reshard()`) marches through the N old (gen-g) units. For each old unit K: (1) create the two new gen-(g+1) units K and K+N (fresh databases); (2) BACKGROUND-COPY: scan old-unit-K's keys, route each to new-K or new-(K+N) by `ChildUnit` (the new hash bit), write into the new units - old-unit-K KEEPS SERVING reads + writes throughout; (3) CATCH-UP + ATOMIC CUT-OVER: briefly pause writes to old-unit-K's key-space ONLY, drain the last writes into the new units, then atomically flip routing for old-K's key-space from old-K (gen g) to new-K / new-(K+N) (gen g+1), and retire old-unit-K. After all N bisect, the cluster is uniformly at gen g+1 (2N units); the 2N units then redistribute across nodes by the Phase 3 lease handoff (reused as-is: the ring re-keys onto the 2N generation-qualified unit ids, and reconcile acquires/releases).

- **Generation-aware routing during the transition.** The added routing state is small: the current generation g + a per-old-unit "has this bisected/cut-over" set. For a key mid-reshard: compute its OLD unit K = `UnitForShardKey(key, N)` at gen g. If K has cut over -> route to its NEW unit `UnitForShardKey(key, 2N)` at gen g+1. Else -> old unit K at gen g. Deterministic; reads/writes land correctly throughout the reshard.

- **NO-ACKED-WRITE-LOST during the bisect (the safety core).** The hazard: a write to old-unit-K arriving DURING the background copy or the catch-up must not be lost. The copy-then-catch-up-then-atomic-cut-over pattern - the same shape as the validated Phase 3 handoff - handles it: writes before cut-over go to old-K and are captured by the catch-up drain; writes after cut-over go to the new units; the brief per-key-space write-pause makes the boundary clean. With R=1 + durable-before-ack, every ACKED write is visible after the reshard. The phase is built to this invariant.

**IN SCOPE: growth by doubling under stable membership.** **OUT OF SCOPE (later phases):** concurrent membership-change + reshard (assume stable membership during a reshard; that interaction is a later concern); halving / shrink (growth/doubling only); the migration tool (per-node -> per-unit, a separate roadmap item); per-unit replication / R>1 and relaxed durability (still R=1, still AwaitDurable=true). The legacy per-node path is not touched.
