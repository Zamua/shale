...
A NEW operator-facing RPC `Reshard` is added to `ShaleNode` (distinct from the existing cluster-INTERNAL `ReshardControl`, which carries individual barrier phases between nodes and is never called from outside the cluster). `ReshardRequest{}` is empty (the only supported reshard is a doubling N -> 2N; no parameters). `ReshardResponse{ from_unit_count, to_unit_count, error }` reports the generation transition or a typed failure. The server handler calls a NEW operator entrypoint `Cluster.TriggerReshard()` (NOT the library `Reshard()`) and maps its result: a clean return reports the doubled `{from, to}` unit counts; the guards surface as the `error` string with equal counts (nothing moved). Carrying the failure in the response field (not a gRPC status) matches the `CommitCAS` / `ApplyBatch` / `ReshardControl` wire convention.

`TriggerReshard()` and the library `Reshard()` share ONE copy of the actual reshard body (`reshardLocked()`, which routes to `reshardCoordinated()` on a multi-node cluster or the single-node bisect otherwise); they differ ONLY in how they acquire `reshardMu`. `Reshard()` BLOCKS on `reshardMu.Lock()` (the established in-process test + embedded-library contract: a second call serializes behind the first and then doubles again against the already-doubled generation). `TriggerReshard()` uses `reshardMu.TryLock()` and REJECTS a concurrent reshard with the typed `ErrReshardInFlight` instead of queuing it: an operator who fires two `shale reshard`s in quick succession must NOT silently get a 4N double-doubling. This is the one piece of behavior the operator path adds; every other safety property is inherited unchanged.

**Who can call it + safety.** The operator-triggerable surface is GATED by a shared-secret authz check (see "Admin-RPC authorization" below): only a caller presenting the configured `SHALE_OPERATOR_SECRET` can issue `Reshard`. The `shale` CLI gains a `shale reshard` subcommand that dials one node, attaches the secret, and issues the RPC. Safety BEYOND authz is inherited from the validated barrier, not re-implemented:

  - **Concurrent reshard rejected.** `TriggerReshard` takes `reshardMu` via `TryLock`; if another reshard already holds it, it returns `ErrReshardInFlight` immediately rather than blocking. The operator retries once the in-flight one finishes; the cluster never double-doubles from two stacked operator triggers.
  - **Unstable membership refuses cleanly.** The coordinated flow snapshots the member IDENTITY set (not just count) and re-checks it before BISECT and before FLIP, ABORTing on any add / remove / count-preserving swap. A reshard issued while membership is in flux refuses cleanly and STAYS at gen g - it never hangs and never half-reshards.
  - **Ceiling check is all-or-nothing.** The up-front `count.Double()` check means a reshard either fully proceeds or fully refuses before any node freezes.
  - **Idempotent-safe to retry after a clean failure.** Every error path (`ErrReshardInFlight`, legacy-mode refusal, ceiling refusal, mid-barrier ABORT) leaves the generation UNCHANGED, so re-issuing the RPC simply re-attempts from the current generation. A success advances the generation by exactly one doubling. (The post-FLIP `ErrReshardStraddle` case - a node flipped before a later node failed - is the documented out-of-scope residual that surfaces as a DISTINCT error string so the operator knows recovery is manual, not a clean retry.)

#### Admin-RPC authorization

The `ShaleNode` gRPC service carries BOTH a data plane (`Put` / `Get` / `Delete` / `ScanPrefix` / `LocalScan` / `CommitCAS` / `ApplyBatch` and the forwarded variants) AND a small set of ADMIN RPCs that cause cluster-wide effects. Without authz, any client that can reach a node's gRPC port can force a cluster-wide FREEZE + doubling reshard. A shared-secret check gates the admin surface.

**The gated (admin) RPC set.** Three RPCs are admin because each triggers a cluster-wide side effect:

  - `Reshard` - the OPERATOR entrypoint; kicks off a cluster-wide freeze + doubling. The primary thing to protect.
  - `ReshardControl` - the cluster-INTERNAL barrier-phase carrier; a forged `FREEZE` could pause writes cluster-wide. Gated because it too causes a cluster-wide effect (and gating it forces the node-to-node coordination to authenticate, below).
  - `ProposeRebalance` - the operator rebalance planner / applier / canceller (`apply` / `cancel` move data and stop in-flight migrations).

Everything else - the entire data plane plus `Topology` / `Stats` / `Ping` / `GenState` / `MigrateRange` - is NOT gated by this interceptor. Full data-plane authentication (per-tenant auth, mTLS) is a v0.9 concern, explicitly out of scope here; this milestone closes ONLY the operator-admin hole. `GenState` and `MigrateRange` are cluster-internal but read-only / handoff-scoped and carry no cluster-wide freeze power, so they stay ungated to keep a joiner's Open path and the lease handoff working without the secret.

**The mechanism: a server-side unary interceptor.** A `grpc.UnaryServerInterceptor` is installed when the node's `grpc.Server` is built (`pkg/shaled/runtime.go:203` - `grpc.NewServer()` gains `grpc.UnaryInterceptor(...)`; today it has none). The interceptor inspects `info.FullMethod`; for a method in the admin set it reads the operator secret from request metadata (gRPC `metadata.MD`, key `x-shale-operator-secret`) and compares it to the configured `SHALE_OPERATOR_SECRET` with `subtle.ConstantTimeCompare` (never `==`, never logged). A mismatch or missing-metadata is rejected with `codes.PermissionDenied` + a generic message (no oracle on whether the key was present vs wrong). Non-admin methods pass through untouched. Streaming admin RPCs are not in the gated set, so a unary interceptor suffices.

**Configuration + the fail-closed rule.** The secret is read once at node start from the `SHALE_OPERATOR_SECRET` env var.

  - **When SET:** the interceptor enforces the constant-time check on every admin RPC. A configured cluster shares ONE secret across all nodes (env on each), so both an operator's `shale reshard` and a coordinator's node-to-node `ReshardControl` present it and the reshard coordinates normally.
  - **When UNSET (fail-closed on the operator path):** the gRPC `Reshard` handler is REFUSED with a clear, actionable error (`configure SHALE_OPERATOR_SECRET to enable operator reshard`) - the operator must opt in by configuring a secret; an unconfigured cluster cannot be resharded over the wire by anyone. This is the fail-CLOSED posture: no secret means no operator-triggerable reshard, rather than an open door.

**Node-to-node coordination must not wedge when configured.** Gating `ReshardControl` means peers must authenticate to each other. The coordinator's outbound barrier calls (`peerClient.ReshardControl`, dialed from `Cluster.sendPhase`, `pkg/cluster/multibackend_reshard_barrier.go:887`) attach the same secret as outbound metadata; the `newPeerClient` dial (`pkg/cluster/remote.go:31`) is the seam that learns the secret (from the same env the node read at start). So in a CONFIGURED cluster every barrier phase authenticates and the reshard coordinates end to end; in an UNCONFIGURED cluster the operator `Reshard` entrypoint already fails closed before any barrier phase is sent, so `ReshardControl` coordination is never reached on the unconfigured path and nothing wedges. The coordinator's SELF participant (`sendPhase` when `m.id == c.cfg.NodeID`) calls the local handler directly with no RPC, so it needs no secret. The interceptor's UNSET behavior for `ReshardControl` specifically must NOT hard-refuse internal coordination on a configured cluster: enforcement keys off "secret configured", and since the operator path is the only entry that reaches `ReshardControl`, an unconfigured cluster never drives the barrier at all.

**In-process callers are UNAFFECTED.** The interceptor sits on the gRPC transport only. The in-process Go methods `Cluster.Reshard()` (blocking, the library/test contract) and `Cluster.TriggerReshard()` (non-blocking, the operator entrypoint the gRPC handler delegates to) are NOT behind the interceptor: an embedded caller (a future `hostthis` that imports `pkg/cluster` directly) always works, with no secret. Only the WIRE entry to `Reshard` is gated.

**Secret hygiene.** The secret is never written to logs, never echoed in an error message, and compared only via `subtle.ConstantTimeCompare`. `.env.example` for any deploy lists `SHALE_OPERATOR_SECRET=` as a known key (the operator supplies the value).
