...
A cluster opts in by setting `Config.ConditionalStore` on an R>1 multi-backend node; `Open` then constructs and seeds the Arbiter (idempotent across the cluster: one node's create-if-absent wins, every other node and any later joiner adopts the already-seeded State). When unset, or at R=1 / legacy, the Arbiter is not constructed and the cluster stays byte-for-byte on the existing reshard paths. The Arbiter is currently INERT: it establishes the agreed epoch object but does not yet drive any routing, copy, or cut-over - the online split protocol it drives (two-generation union routing, ack-over-parent dual-writes, envelope-aware apply-if-newer copy, the durable per-unit caught-up cut-over marker, marker-gated ordered parent removal) lands in subsequent phases and is specified here as it is built.
