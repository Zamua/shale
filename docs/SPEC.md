...
**The two halves of the machine.**
  ...
  - ON THE NEW OWNER (the node GAINING the position): `Absent -> Acquiring -> Ready -> Owned`. `Acquiring`: desired by the LIVE ring (which has rotated to target this node) but not yet mounted; `OpenReplicaUnit` is running the slow MinIO mount + per-replica fence. Because routing now targets this node yet it has no local mount, a routed op is NOT refused with `errUnitAcquiring`: instead the new owner FORWARDS the op to the PREDECESSOR (the old owner, learned from the pre-change replica set in the membership/ring delta) over RPC, read AND write transparent, and returns the predecessor's result. The forward reuses the existing `clientFor` + forwarded-op plumbing (`PutForwarded`, `forwardGet`, the CAS-apply forward), just aimed at the predecessor's address. If the predecessor is unreachable, the new owner FALLS THROUGH to `errUnitAcquiring` (the Option-A retry belt). THIS IS THE OVERLAP WINDOW, and mount time here is irrelevant to availability. `Ready`: `OpenReplicaUnit` returned at a fenced epoch E (strictly above the durable manifest writer-epoch); the `mountMap[ru]` entry is inserted under mountMu in ONE critical section. THE INSTANT THAT ENTRY IS VISIBLE, this node serves routed ops LOCALLY and STOPS forwarding (THE MOUNT FLIP), and sends `ReplicaHandoffReady(ru, E)` to the predecessor. Then `Ready -> Owned` (the transient phase entry is dropped; steady state is just a mounted entry).
...
**Resolved review findings.** The adversarial review (`docs/design/overlap-handoff-review.md`, kept as history) found holes the behavior above now closes:
...
- [~] **v0.8** (in progress) - per-shard lease-handoff storage: copy-free rebalance + online doubling resharding. Replaces the per-node single-database model (one slatedb per node, rebalance copies keys) with a FIXED power-of-two count N of self-contained storage-unit databases whose OWNERSHIP is a lease (the ring is re-keyed onto unit ids; handoff is close-on-old-owner / open-at-epoch+1-on-new-owner, zero bytes copied). N is bounded by per-engine memtable memory; growth by doubling makes resharding an online per-unit bisect with no global re-partition. Full design in the "Per-shard lease-handoff storage" section above. Sub-tasks:
  ...
  - [ ] Overlap handoff (Phase 2e, Option B): the OLD owner of a moving `ReplicaUnit` KEEPS SERVING until the NEW owner has fully acquired (mounted + durably fenced + ready), and during the overlap the NEW owner FORWARDS routed ops to the predecessor (the OLD owner, from the membership delta) so the ring rotation does not strand the position; the mount flip happens atomically at the new owner's `mountMap[ru]` insert; the old owner releases exactly once when the new owner is READY (the `ReplicaHandoffReady` RPC is authoritative, sent only after the flip; the durable manifest writer-epoch is a liveness HINT that triggers a readiness re-verify, never a bare-epoch release). Availability no longer depends on mount time (a 30s mount is a non-event). Expressed as ONE per-`ReplicaUnit` ownership-transition state machine (`Acquiring/Ready` on the gainer, `Draining/Releasing` on the loser, over the `Owned/Absent` poles) replacing the position-change branch of `reconcileReplicaUnits`, with `mountMap` / phase map / `HandoffState` re-keyed by `ReplicaUnit`; pure transition types in `pkg/storageunit` (`handoff.go`), the controller in `pkg/cluster` (`multibackend_overlap.go`), `DurableEpochReplica` lifted onto the `ReplicaBackendFactory` interface. Open implementation-must-verify: the new owner's open must replay the full durable WAL tail (pin with a write-then-fence test; slatedb-go is an opaque FFI binding). Acceptance: an in-process loss-oracle gate with an ARBITRARILY SLOW `OpenReplicaUnit` asserts zero acked loss AND acked/attempted ~100% even under a multi-second mount (where Option A alone collapses), demonstrated to FAIL on a forced clean-cut with Option-A retry also off. Option A's retry is retained as the safety net for the residual unserved instant (predecessor unreachable, crash mid-handoff, pure-new-mount convergence); A and B compose. See "v0.8 Phase 2e" above.
