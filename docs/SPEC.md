...
### v0.8 Phase 2g: CAS-guarded serving-marker lease (safe full-cluster restart)

Phase 2f's boot-defer (read the serving marker before opening; SKIP a position a peer is already serving so the open does not fence the live peer) is correct for the case it was built for: ONE node restarting while its peers stay up. That node reads its peers' LIVE markers and defers, and the steady-state reconcile hands the positions off after convergence. But a FULL-CLUSTER restart - every pod going down and coming back at once (a rollout, a power event, a cluster-wide redeploy) - WEDGES under the same boot-defer. The root cause is that the Phase 2e/2f serving marker, as specified above, CANNOT TELL A LIVE OWNER FROM A DEAD ONE, and is NOT a single-winner lock:

  - **The marker payload is a BARE EPOCH** (a decimal-string monotonic counter, `writeServingMarker`/`readServingMarker` above). It carries NO owner identity and NO timestamp. So a booting node that reads a present marker cannot distinguish "a LIVE peer is serving this position right now" from "a peer that is now DEAD left this marker behind before it died." On an all-pods restart EVERY position's marker is present-but-stale (its writer is gone), so every booting node defers to ghosts and mounts NOTHING ("0 mounted, N DEFERRED"); the cluster never re-forms ownership.
  - **The marker is written LAST-WRITE-WINS, guarded only by a monotonic floor** (never lower the recorded epoch), via a plain `PutObject`. It is NOT a compare-and-swap, so it cannot serve as an ownership LOCK. The ACTUAL ownership handoff today is "open the slatedb writer," and a writer-open BUMPS THE EPOCH = a fence (intrinsic to slatedb's `DbBuilder.Build`). So when the reconcile races on multiple nodes during gossip convergence, two nodes both OPEN the same position and FENCE-FIGHT, leaving positions wedged. The marker observes the race; it does not arbitrate it.

Phase 2g fixes BOTH by turning the serving marker into a CAS-GUARDED LEASE with embedded liveness. Three parts:

**A. Put liveness in the marker (the lease record).** The marker payload is no longer a bare epoch. It becomes a small struct:

```
ServingLease {
    Epoch       Epoch   // the owner's EXACT open epoch (from OpenReplicaUnit), as today
    OwnerNodeID string  // c.cfg.NodeID of the node currently serving this position
    HeartbeatTS int64   // Unix-nanos the owner last refreshed the lease (wall clock)
}
```

`Epoch` keeps its exact meaning and monotonicity from Phase 2e (the precise factory-returned open epoch, captured once at mount, immutable for the life of the mount; the strict-`>` release gate and the durable-fence-hint distinction are UNCHANGED). `OwnerNodeID` is sourced from `c.cfg.NodeID` (the same identity the ring + membership use). `HeartbeatTS` is refreshed PERIODICALLY by the serving owner (a lightweight CAS-rewrite of its own lease at the same epoch, on the existing reconcile / settle cadence - the lease window is several reconcile ticks, so a single missed refresh does not expire a live owner). The wire payload is a tiny JSON object (still a sibling object of the position's slatedb database, still written/read WITHOUT opening the database, so the read stays fence-free). Backward note: this is a NEW payload shape; a Phase 2e bare-epoch marker is treated as "epoch known, owner unknown, heartbeat absent" = STALE by predicate (B), which is the safe default (a node will CAS-claim it rather than defer to a ghost).

**B. The staleness predicate (when a present marker is NOT deferred to).** A present lease `L` is STALE - and therefore MUST NOT be deferred to at boot, and MAY be CAS-stolen on acquire - when EITHER:

  - `L.OwnerNodeID` is NOT in the LIVE gossip member set (`membership.Snapshot()`), i.e. the recorded owner is not a currently-alive peer; OR
  - `now - L.HeartbeatTS > LeaseWindow` (the heartbeat is older than the lease window, default a small multiple of the reconcile interval, operator-tunable), i.e. even if the owner ID is in the set the lease has not been refreshed recently enough to be trusted as live.

A lease that is present AND fresh AND owned by a live member is LIVE: a booting / acquiring node DEFERS to it exactly as Phase 2f does today (do not open, do not fence). The owner-in-live-set check is the FAST path that fixes the full-restart wedge (after an all-pods restart, NO recorded owner is in the freshly-formed member set until the owners re-acquire, so every marker is stale, so no node defers to a ghost). The heartbeat-age check is the BACKSTOP for the imperfect case where a stale owner ID lingers in gossip (a crashed node not yet reaped) - its lease ages out of the window and is reclaimed. Membership is the primary signal, heartbeat is the seatbelt.

**C. Claim-by-CAS, not by opening (the single-winner acquire).** To ACQUIRE a position, a node no longer just opens the writer and lets the fence sort out contention. It first CONDITIONALLY WRITES the lease object:

  1. READ the current lease object AND its object-store ETag (`ReadServingLease(ru) -> (lease, etag, present)`).
  2. If a lease is present and LIVE by predicate (B), DO NOT acquire - defer (a live owner holds it).
  3. If absent, or present-but-STALE, attempt a COMPARE-AND-SWAP of the lease object: `PutObject` with the object-store conditional header (`If-Match: <observed etag>` for a present-stale lease, `If-None-Match: *` for an absent one) to `{Epoch: <my intended open epoch>, OwnerNodeID: <me>, HeartbeatTS: now}`. This is exactly the conditional-write CAS the deployment's `pgsty/minio` fork provides (and the reason that fork is pinned). The CAS has EXACTLY ONE WINNER per position per contended round.
  4. The CAS WINNER - and ONLY the winner - then OPENS the writer (`OpenReplicaUnit`). Because it is the sole winner, its open / fence is UNCONTESTED: there is no peer simultaneously opening the same position, so the fence harms nobody. After the open returns the exact epoch `E`, the winner REWRITES its lease (CAS again, If-Match its own just-won ETag) to carry the true open epoch `E` (the claim wrote the INTENDED epoch; the post-open rewrite records the ACTUAL one), and begins refreshing `HeartbeatTS` on the reconcile cadence.
  5. Every CAS LOSER sees the conditional write rejected (the ETag moved / the object now exists): it BACKS OFF, does NOT open, does NOT fence, and re-evaluates on the next reconcile tick (by then the winner's fresh live lease makes the loser DEFER under (B)). A loser NEVER opens the writer, so the fence-fight is impossible BY CONSTRUCTION.

**The convergence gate (decide off a settled ring).** A node MUST NOT CAS-steal a stale-looking lease from a peer that is actually ALIVE but whose gossip simply has not arrived yet. So the acquire is GATED on gossip membership having CONVERGED: the live member set (`membership.Snapshot()`) must have been STABLE (unchanged) for a short settle window (a few seconds / a few reconcile ticks) before a node will treat a lease's owner-not-in-set as grounds to CAS-steal. Until the ring settles, a node still DEFERS to a present lease (conservative: prefer a brief unserved window over stealing from a live owner mid-gossip). The lease `HeartbeatTS` (part A) is the BACKSTOP for the residual case where convergence is imperfect or a settled-looking set is wrong: a genuinely live owner keeps its heartbeat fresh, so even a hasty CAS-steal attempt loses to (or is corrected by) the live owner's refresh, and the heartbeat-age predicate keeps a truly-dead owner reclaimable. Convergence is the primary guard; heartbeat is the backstop.

**THE INVARIANT (stated explicitly).** *A replica position's slatedb writer is opened by AT MOST ONE node at a time, and that node is the UNIQUE CAS WINNER of the marker lease.* Opening the writer is gated behind winning the lease CAS; the CAS has one winner per contended round; a loser never opens. The fence (which slatedb performs inside `Build`) therefore never arbitrates a contended open - the CAS already did, before any open - so two nodes can no longer fence-fight a position. This is STRICTLY STRONGER than the Phase 2f boot-defer (which only prevented a booting node from fencing an already-serving peer, and only worked when that peer was live-and-gossip-visible): the lease CAS makes single-writer hold through the ALL-AT-ONCE restart, where every marker is stale and the old boot-defer wedged.

**What this changes in the existing machinery (REUSED vs CHANGED).** REUSED unchanged: the strict-`>` epoch release gate, the durable-fence-epoch-as-weaker-hint distinction, ORDERED REMOVAL, the `ReplicaUnit` keying, the dual-write union routing, the `DrainForLeave` drain gate - Phase 2e's correctness arguments all stand (the lease carries the SAME `Epoch` they gate on; it just adds owner + heartbeat fields and a CAS-guarded write). CHANGED: (1) the marker payload struct (bare epoch -> `ServingLease`); (2) `WriteServingMarker` becomes a CAS-guarded lease write and the acquire path threads the read-ETag -> CAS -> open -> rewrite sequence (the `ReplicaFactory` interface in `pkg/storageunit/factory.go` gains the ETag-returning read and the conditional write - `ReadServingLease(ru) -> (lease, etag, present, err)` and `CasServingLease(ru, expectedETag, lease) -> (newETag, ok, err)`, the bare-epoch `WriteServingMarker`/`ReadServingMarker` either grow the struct or are superseded); (3) the boot-mount and reconcile-acquire defer/skip decisions consult predicate (B) (owner-live + heartbeat-fresh) instead of "marker present at epoch > 0"; (4) the serving owner refreshes its heartbeat on the reconcile cadence; (5) acquire is gated on the membership-convergence settle window. The R=1 lease-handoff (Phase 3) and the legacy per-node path stay UNTOUCHED (Phase 2g lives behind `multiReplicated()`, like the rest of Phase 2e/2f).

**The slate (MinIO) CAS mechanism.** The conditional write is the S3 conditional-PUT the `pgsty/minio` fork supports: `If-None-Match: *` to create-only (claim an absent lease) and `If-Match: <etag>` to swap-only (steal a specific stale lease / refresh own lease). A rejected conditional returns the precondition-failed error, which the acquire path classifies as "lost the CAS, back off." The read returns the object's ETag alongside the payload so the next CAS can condition on it. This is exactly the capability called out at `backends/slate/factory.go` (the "A CAS (If-None-Match / version) would tighten it" comment) - Phase 2g is that tightening, now load-bearing for the full-restart case rather than merely a hardening.

**The in-memory test double (sharedfactory) MUST simulate ETag CAS.** `internal/sharedfactory/sharedfactory.go` is the in-process double the acceptance test runs against (no real MinIO). It must model the lease object as `{payload, etag}` under its mutex, where `etag` is a monotonic version counter bumped on every successful write. `ReadServingLease` returns the current `(payload, etag, present)`; `CasServingLease(ru, expectedETag, lease)` succeeds (and bumps the etag) ONLY when the stored etag equals `expectedETag` (or, for the create-only variant, only when no object is present), and otherwise returns ok=false WITHOUT mutating - so two concurrent claimers race exactly one winner, reproducing MinIO's conditional-PUT semantics deterministically in-process. This is what lets the regression test exercise the single-winner protocol without MinIO.

**Acceptance (the break-demo).** A new in-process gate (sharedfactory + memory backend, NO slatedb tag, NO MinIO) stands up an R=2 multi-backend cluster, writes a recorded BASELINE, then RESTARTS EVERY NODE AT ONCE (drops all clusters and re-creates them against the same shared backing, so every position's lease is present-but-stale, its writer gone). It asserts: (1) the cluster RE-FORMS ownership - every position ends mounted on exactly one node (no "0 mounted, N DEFERRED" wedge); (2) every baseline key is readable with its exact value after the restart (no acked-write loss); (3) NO position is opened by two nodes (the single-writer invariant - assert via the factory that each `OpenReplicaUnit` for a given epoch round had one winner / no fence-fight); (4) writes resume to full R=2 after convergence. It is kept honest by a BREAK DEMONSTRATION: with the lease CAS reverted to the bare-epoch monotonic-floor marker (the Phase 2f behavior), the same all-at-once restart WEDGES (every node defers to ghosts -> 0 mounted) AND/OR fence-fights (two nodes open the same position), proving the CAS lease is what re-forms the cluster rather than rubber-stamping it. A second case restarts ONE node while peers stay up and asserts it STILL defers to the live peers' fresh leases (Phase 2f's original guarantee is preserved, not regressed).
...
The v0.8 multi-node reshard above is imperative (an operator calls `Reshard()`, a coordinator drives a cluster-wide FREEZE barrier) and grows only (doubling). v0.9 replaces that on the R>1 path with a DECLARATIVE, DECENTRALIZED reshard: declare the desired unit count (the way N pods are declared) and the cluster reconciles toward it ONLINE, growing (split, N -> 2N) or shrinking (merge, 2N -> N), with NO cluster-wide freeze and NO elected coordinator. The full design is `docs/decentralized-reshard-design.md`; this section folds in incrementally as each phase lands. Scope: R>1 multi-backend ONLY (the R=1 multi-node path keeps the coordinated freeze barrier above; the single-node path keeps the inline online bisect). Power-of-two double/halve only (clean bisection by one hash bit, no all-to-all reshuffle). `count = 0` is invalid and rejected.
...
A cluster opts in by setting `Config.ConditionalStore` on an R>1 multi-backend node; `Open` then constructs and seeds the Arbiter (idempotent across the cluster: one node's create-if-absent wins, every other node and any later joiner adopts the already-seeded State). When unset, or at R=1 / legacy, the Arbiter is not constructed and the cluster stays byte-for-byte on the existing reshard paths.
...
- [~] **v0.8** (in progress) - per-shard lease-handoff storage: copy-free rebalance + online doubling resharding. Replaces the per-node single-database model (one slatedb per node, rebalance copies keys) with a FIXED power-of-two count N of self-contained storage-unit databases whose OWNERSHIP is a lease (the ring is re-keyed onto unit ids; handoff is close-on-old-owner / open-at-epoch+1-on-new-owner, zero bytes copied). N is bounded by per-engine memtable memory; growth by doubling makes resharding an online per-unit bisect with no global re-partition. Full design in the "Per-shard lease-handoff storage" section above. Sub-tasks:
  ...
  - [ ] CAS-guarded serving-marker lease (Phase 2g, safe full-cluster restart): turn the serving marker from a bare-epoch monotonic-floor `PutObject` into a CAS-guarded LEASE `{Epoch, OwnerNodeID, HeartbeatTS}` so an all-pods-at-once restart re-forms ownership instead of wedging. Three parts: (A) embed liveness - owner identity (`c.cfg.NodeID`) + a periodically-refreshed heartbeat in the marker; (B) staleness predicate - a present lease is STALE (not deferred to, may be CAS-stolen) when its owner is NOT in the live gossip member set OR its heartbeat is older than the lease window; (C) claim-by-CAS - to acquire a position a node reads the lease+ETag, and if absent/stale CAS-writes (`If-None-Match: *` / `If-Match: <etag>` via the `pgsty/minio` conditional-PUT) `{me, my epoch, now}`; the single CAS winner (and only it) opens the writer (uncontested fence), losers back off without opening. Gated on membership-convergence (settled member set) so a slow-gossip live peer is not CAS-stolen; heartbeat is the backstop. INVARIANT: a position's slatedb writer is opened by at most one node at a time, the unique CAS winner of the lease - so the fence-fight is impossible by construction. Interface: `ReplicaFactory` gains `ReadServingLease(ru) -> (lease, etag, present, err)` + `CasServingLease(ru, expectedETag, lease) -> (newETag, ok, err)`; the in-memory `sharedfactory` simulates ETag CAS (monotonic version counter, single winner per contended round) so the gate runs without MinIO. REUSED unchanged: the strict-`>` epoch release gate, the durable-fence hint, ORDERED REMOVAL, `ReplicaUnit` keying, union routing, `DrainForLeave`. Behind `multiReplicated()`. Acceptance: an in-process all-nodes-at-once restart gate asserts the cluster re-forms ownership (every position mounted on exactly one node, no "0 mounted, N DEFERRED" wedge), zero acked loss, and no position opened by two nodes; the break-demo reverts the lease CAS to the bare-epoch marker and shows the wedge/fence-fight; a second case proves a single-node restart still defers to live peers. See "v0.8 Phase 2g" above.
