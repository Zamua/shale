...
- `dbNameReplica` is defined as a sibling of `dbName` on `BackingConfig`, reusing `dbName(ru.Unit)` verbatim so the unit prefix can never drift between the R=1 and R>1 encodings.
...
The R=1 factory tests are unchanged. New gated (`slatedb` + `integration`) tests, mirroring the R=1 ones at a replica position:
