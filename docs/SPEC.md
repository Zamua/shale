...
Phase 2d (Option A) keeps R>1 writes available across a membership change by RETRYING through the acquiring window, bounded by `WriteTimeout`. That makes availability a function of mount-time-vs-timeout: a single slatedb mount from MinIO can exceed the 5s budget under a big-bang scale-out (~16 databases remounting at once), and the measured big-bang 3 -> 12 ack rate falls to ~54%. Phase 2e (Option B) removes the dependency on mount time entirely: the OLD owner KEEPS SERVING a moving (unit, replica) until the NEW owner has FULLY acquired it (mounted + durably fenced + ready); the two ownerships OVERLAP during the transition; routing flips only once the new owner signals ready, epoch-fenced, with no instant where the position is unserved. Availability then no longer depends on mount time at all. A 30s mount is a non-event. The detailed design (state diagram, sequence, package layout) is in `docs/design/overlap-handoff.md`; this section is the canonical behavior.

**The clean shape: ONE per-(unit, replica) ownership-transition state machine, not scattered flags.** A handoff today is two UNSEQUENCED halves on two nodes: the losing node drops the position from its desired set and RELEASEs; the gaining node adds it and ACQUIREs. Nothing orders them, so there is a window where NEITHER serves (the bug). Phase 2e replaces the position-change branch of `reconcileReplicaUnits` (the RELEASE-then-ACQUIRE) with an ACQUIRE-then-RELEASE driven by an explicit phase per side. The position is the unit of ownership: `(GenUnit, replica-position)`, node-independent (the same durable prefix `dbNameReplica(ru)` is what the old owner has open and what the new owner opens), which is exactly what makes overlap possible. The phase map holds ONLY in-flight positions (keyed by `GenUnit`, sibling of the existing `replicaPos` map, mountMu-guarded); a position's steady state is encoded by mountMap PRESENCE + phase ABSENCE, so the enum carries only the four transient phases. No `isDraining` / `hasFlipped` / `pendingRelease` booleans: the phase value plus mountMap presence IS the complete state, and every transition is a single guarded edge.

**The two halves of the machine.**

  - ON THE NEW OWNER (the node GAINING the position): `Absent -> Acquiring -> Ready -> Owned`. `Acquiring`: desired by the live ring but not yet mounted; `OpenReplicaUnit` is running the slow MinIO mount + per-replica fence. Routing does NOT resolve this node for the position yet (mountMap has no entry, so `localBackendForKey` returns `ok=false` and the node refuses with the retryable `errUnitAcquiring`, exactly as today). The OLD owner is still serving. THIS IS THE OVERLAP WINDOW, and mount time here is irrelevant to availability. `Ready`: `OpenReplicaUnit` returned at a fenced epoch E (strictly above the durable manifest writer-epoch); the mountMap entry + `replicaPos` are inserted under mountMu in ONE critical section. THE INSTANT THAT ENTRY IS VISIBLE, this node answers routed ops for the position. THE mountMap INSERT IS THE FLIP. Then `Ready -> Owned` (the transient phase entry is dropped; steady state is just a mounted entry, identical to today).

  - ON THE OLD OWNER (the node LOSING the position): `Owned -> Draining -> Releasing -> Absent`. `Draining`: the reconcile computed this position is no longer desired here, but the old owner does NOT delete the mountMap entry; it KEEPS SERVING and records `Draining` + arms a release-check. `Releasing`: the release-check observed the new owner has fenced ahead, so the old owner compare-and-deletes its mountMap entry (only if it still points at the same backend, reusing the `evictStaleMount` CAS shape) and calls `CloseReplicaUnit(ru)` EXACTLY ONCE, then `Releasing -> Absent`.

**The cut (the exact epoch-fence point).** The handoff cut is a single instant on the per-replica durable manifest: the epoch E at which the NEW owner opened (`OpenReplicaUnit` opens at `max(intended, durableEpochReplica+1)`, and that open WRITES the bumped epoch to the manifest). Below E, the OLD owner is the authoritative writer; at or above E, the NEW owner is. The OLD owner's own writes are fenced the moment it attempts one past E (slatedb `CloseReasonFenced`), so even a buggy late write on the old side fails rather than corrupting the position. Routing flips at the NEW owner's mountMap insert, which happens strictly AFTER the fence (the open completed first), so there is never an instant where BOTH nodes' mountMap resolves the position AND both are unfenced.

**The signal: the durable manifest writer-epoch (primary), an RPC wake (fast-path only).** There is NO new central controller and NO new coordination protocol for correctness. Each node runs `reconcileReplicaUnits` independently off the existing debounced settle timer, driven by the same membership topology events; the ring is the shared truth and both nodes independently conclude the position moves. The cross-node signal is the DURABLE per-replica manifest writer-epoch, readable WITHOUT opening the database via `DurableEpochReplica(ru)` (the slate `Backing` already exposes it; Phase 2e lifts it onto the `ReplicaBackendFactory` interface so the cluster reads it through the domain seam). Two signal paths, priority order:

  1. PRIMARY (durable, crash-proof, no new wire type): the old owner's `Draining` release-check reads `DurableEpochReplica(ru)`. When it exceeds the epoch the old owner holds the database open at, the new owner has provably fenced ahead and is the authoritative writer, so the old owner releases. This works even if the two nodes never talk directly, and it is the GROUND TRUTH.
  2. FAST-PATH (latency only): on reaching `Ready`, the new owner sends a one-shot best-effort `ReplicaHandoffReady(ru, E)` RPC to the old owner (reusing the existing `clientFor` / rpc-server plumbing), which just WAKES the old owner's release-check early instead of waiting for the next poll tick. Losing this RPC costs latency, never correctness, because path 1 is the ground truth. The old owner's `Draining` release-check is otherwise armed as a bounded periodic re-check on the existing settle / self-heal cadence.

**The slate manifest seal (consistent mount under overlap).** The new owner must mount a view containing every write the old owner ACKED. This holds without a new flush protocol because: (1) the slate replica opens with `AwaitDurable=true`, so every ACKED write is durable in the WAL/SST BEFORE its ack returns (durable-before-ack); the new owner opening `dbNameReplica(ru)` reads that durable state. (2) The fence is on the durable manifest writer-epoch; the new owner's open reads the manifest and lands above it. The difference Option B introduces over Option A is that the new owner now acquires WHILE the old owner is still writing (in A the old owner sealed via `CloseReplicaUnit` BEFORE the new owner acquired). That is safe: the new owner does not need the old owner's UN-acked in-flight writes (the client never got success for those and retries), only every ACKED write, and those are durable at the instant they were acked, which is strictly BEFORE the fence. A write the old owner acks AFTER the new owner has fenced is impossible: that write would ride the old owner's now-fenced handle and fail (`CloseReasonFenced`), so it is never acked. There is no "acked on old, invisible to new" gap. The old owner's `CloseReplicaUnit` at `Releasing` still seals (Db.Shutdown flushes), but for acked writes it is belt-and-suspenders.

**THE FIVE CORRECTNESS GUARANTEES (argued explicitly).**

  1. NO LOST WRITES (a write acked before/during/after the flip is durable + visible after it). Before the flip the write lands on the OLD owner (`AwaitDurable=true` -> durable before ack); the new owner opens the same durable prefix and sees it. During the flip a write routes to whichever side mountMap currently resolves; if it lands on the old owner pre-flip it is durable-before-ack and the new owner reads it; if it lands on the new owner post-flip it is in the new owner's now-authoritative database. After the flip every write is on the new owner. At no instant is there a write that is acked on one side and invisible to the resolving side, because acks require durability and the fence orders the two sides on the durable manifest.

  2. NO DOUBLE-APPLY / NO SPLIT-BRAIN (old and new never both accept conflicting writes past the fence). The fence point is epoch E on the durable manifest. Exactly one side holds the database open at the highest epoch at any instant; the other side's writes fail with `CloseReasonFenced`. Routing resolves the position to exactly one mountMap entry at a time (the old owner's until the new owner inserts, the new owner's after), and the inserting side is the FENCED-AHEAD side, so the routed-to side is always the authoritative writer. A write that races the flip lands on the losing side's now-fenced handle, fails, is evicted (`evictStaleMount` -> `errUnitAcquiring`), and is retried onto the winning side. Apply-if-newer (LWW stamp) makes even a reordered or replayed envelope a no-op, so there is no conflicting double-apply.

  3. CRASH SAFETY. (a) NEW crashes mid-acquire (Acquiring, before the flip): the durable epoch never advances, the old owner's release-check never fires, the old owner stays `Draining` and KEEPS SERVING. No data loss, no unavailability, just no progress; the next reconcile re-derives the ring and reassigns the position. (b) OLD crashes while `Draining` (before NEW is Ready): the routing-resolved side is momentarily down for THIS one position, but at R>1 the other replicas still serve toward W, and the new owner is already acquiring from the same durable state (durable-before-ack means it sees every acked write); this degrades to the Option-A window for one position only, bounded by the new owner's mount, and only when the churn also kills the old owner mid-flight. (c) NEW crashes AFTER Ready but before OLD released: the old owner is still mounted but fenced (its writes fail); the position is owned-but-unmounted per the ring; the next reconcile reassigns it (new owner gone) and the old owner re-acquires at a higher epoch or hands to a third node; no acked write is lost (all durable), and the old owner's stale mount is torn down by `evictStaleMount` on its next fenced write.

  4. SLOW MOUNT IS A NON-EVENT. The entire `Acquiring` window is invisible to availability: the old owner serves the position throughout, and the new owner's mount can take 30s without any routed op failing. Routing flips only at the `Ready` insert, which is strictly after the mount completes, so a slow mount delays the flip but never opens an unserved window. This is the property Option A could not give (its availability was bounded by mount-vs-`WriteTimeout`).

  5. CLEAN TEARDOWN (the old owner releases EXACTLY ONCE, only after the flip). The old owner transitions `Draining -> Releasing` only when it has observed the new owner fenced ahead (durable epoch crossed, or the RPC wake confirmed it). At `Releasing` it compare-and-deletes its mountMap entry (the `evictStaleMount` CAS guards against deleting a re-mounted entry) and calls `CloseReplicaUnit(ru)` once; the phase machine forbids a second `Releasing` edge from the same `Draining` (an illegal transition is rejected), so the release is exactly-once even if both the poll and the RPC fire.

**OPEN CORRECTNESS REVIEW (must close BEFORE implementation).** An adversarial design review (`docs/design/overlap-handoff-review.md`) found three holes the design above does not yet close; the behavior here is NOT final until they are resolved in this section:

  - (P0) ROUTING DOES NOT FOLLOW THE MOUNT. The write/read fan-out targets the replica set the LIVE ring resolves (`replicasForKey` -> `unitReplicas` -> `LocateKeyN`, recomputed per op), NOT the mount map. Once membership converges the moving position's replica set no longer contains the OLD owner, so nothing routes to the OLD owner's still-mounted backend and availability still bottoms out on Option A's `WriteTimeout`-bounded retry. The "old owner keeps serving" claim only delivers availability if the fan-out is made OVERLAP-AWARE: it must address the UNION of the pre-change and post-change replica sets for a position mid-transition (with W computed against the stable R, not the expanded count), OR the not-yet-ready new owner must FORWARD routed ops to the draining old owner until its mount completes. There are TWO distinct flips: the RING rotation (which nodes are asked, flips at convergence BEFORE the mount) and the MOUNT flip (does the resolved node have the backend open, flips at `Ready`); Option B's correctness argument covers the mount flip, but availability is governed by the ring rotation, which the design must explicitly handle.

  - (P0) THE MOUNT / PHASE MAPS MUST BE KEYED BY `ReplicaUnit`, NOT `GenUnit`. `mountMap` (`map[GenUnit]Backend`) and `replicaPos` (`map[GenUnit]uint8`) hold ONE backend + ONE position per unit per node. Overlap requires a single node to hold the OLD position (draining) AND the NEW position (acquiring/ready) of the SAME unit at once whenever a ring change shuffles this node's position within a unit's replica list (common at R>1). The per-unit maps cannot represent that, and "phase value + mountMap presence is the complete state" is false while presence is per-unit. The mount map, position map, the new phase map, and the pure `HandoffState` must all be re-keyed by `ReplicaUnit` (the durable identity already is), with the ripple through `localBackendForKey` / `localWriteBackendForKey` / `evictStaleMount` / `mountedBackends` specified.

  - (P0/P1) THE RELEASE TRIGGER MUST BE "NEW OWNER READY," NOT "DURABLE EPOCH ADVANCED." `OpenReplicaUnit` writes the bumped epoch to the manifest at FENCE time (`fenceEpochReplica`), strictly BEFORE the slow mount and well before `Ready`. So a new owner that fences and then crashes mid-mount advances the durable epoch WITHOUT ever serving the position; an old owner releasing on a bare epoch-cross then leaves nobody serving (contradicting crash-case 1's "durable epoch never advances" claim, which only holds for a crash BEFORE fence). The safe ground truth is a positive READINESS signal (the `ReplicaHandoffReady` RPC is sent only AFTER the mount flip and IS authoritative; the durable-epoch poll is a liveness HINT that must trigger a re-verify, not an unconditional release). The signal-priority ordering in this section is inverted and must be corrected: readiness is the cut, epoch advance alone is not.

  The review also flags (P1) that the consistent-mount seal argument must invoke WAL-replay-on-open explicitly (a write durable in OLD's WAL but not yet in the manifest snapshot NEW reads must still be recovered by NEW's open, else it is a lost write), (P1) that R=2/W=2 (write-all) crash-of-OLD-mid-Draining degrades to a mount-bounded window for ANY single old-owner crash (not the rare double-fault the crash-safety prose implies), and (P1) the `drainCheck` lock discipline (the `DurableEpochReplica` MinIO I/O must run OUTSIDE `mountMu`; the phase compare-and-advance + the mountMap compare-and-delete must be ONE critical section so the poll and the RPC wake reach `Releasing` exactly once). See the review doc for the full findings, constructed interleavings, and recommended amendments.

**THE OVERLAP-HANDOFF AVAILABILITY GATE (the acceptance test).** A new in-process loss-oracle test (`tests/integration/overlap_handoff_test.go`, sharedfactory + memory backend, NO slatedb tag, NO MinIO) is the acceptance bar. It stands up a multi-backend R=2 cluster on a per-replica shared-backing factory whose `OpenReplicaUnit` can be made ARBITRARILY SLOW (a multi-second mount injection that models a real MinIO mount exceeding `WriteTimeout`), writes a recorded BASELINE, then runs a CONTINUOUS WRITER through a 3 -> N membership change while the slow mounts are in flight, and asserts BOTH (a) THE ORACLE - every baseline key and every ACKED probe key is readable with its exact value from every node, ZERO acked loss; AND (b) THE ACK RATE - acked / attempted is ~100% EVEN with the slow mount (where Option A alone would drop to ~50-54% because the retry budget exhausts on the mount). It is kept honest by a BREAK DEMONSTRATION that disables the overlap (forces the clean-cut RELEASE-then-ACQUIRE): the same slow-mount membership change then drives the ack rate WELL below the threshold, proving the overlap is what holds availability rather than rubber-stamping it. The record-only-acked discipline keeps (a) and (b) independent: durability must hold even at a low ack rate (the old behavior was lossless-but-unavailable), and the fix is the ack rate climbing to ~100% under a slow mount WITHOUT regressing durability.

**IN SCOPE: overlap handoff for an R>1 (and R=1) multi-backend membership change** - the old owner serves until the new owner is fully acquired + durably fenced + ready, ownership flips atomically at the new owner's mountMap insert, the old owner releases exactly once after observing the durable fence cross. Availability no longer depends on mount time. Expressed as ONE per-(unit, replica) ownership-transition state machine (`Acquiring/Ready/Draining/Releasing` over the `Owned/Absent` poles). **OUT OF SCOPE (unchanged):** re-replicating a unit's BYTES onto a brand-new replica position with no durable copy (anti-entropy re-replication is a separate phase); resharding / doubling at R>1 (Phase 4 is R=1); the legacy per-node path and the R=1 lease-handoff (Phase 3) are UNTOUCHED (Phase 2e lives behind `multiReplicated()`). Option A's `WriteTimeout`-bounded retry (Phase 2d) is RETAINED as the safety net for the residual cases (crash mid-handoff, pure-new-mount initial convergence); A and B compose.
...
- [~] **v0.8** (in progress) - per-shard lease-handoff storage: copy-free rebalance + online doubling resharding. Replaces the per-node single-database model (one slatedb per node, rebalance copies keys) with a FIXED power-of-two count N of self-contained storage-unit databases whose OWNERSHIP is a lease (the ring is re-keyed onto unit ids; handoff is close-on-old-owner / open-at-epoch+1-on-new-owner, zero bytes copied). N is bounded by per-engine memtable memory; growth by doubling makes resharding an online per-unit bisect with no global re-partition. Full design in the "Per-shard lease-handoff storage" section above. Sub-tasks:
  ...
  - [ ] Overlap handoff (Phase 2e, Option B): the OLD owner of a moving (unit, replica) KEEPS SERVING until the NEW owner has fully acquired (mounted + durably fenced + ready); routing flips atomically at the new owner's mountMap insert; the old owner releases exactly once after observing the new owner's durable manifest writer-epoch cross above its own open epoch (with a best-effort `ReplicaHandoffReady` RPC as a latency-only fast-path). Availability no longer depends on mount time (a 30s mount is a non-event). Expressed as ONE per-(unit, replica) ownership-transition state machine (`Acquiring/Ready` on the gainer, `Draining/Releasing` on the loser, over the `Owned/Absent` poles) replacing the position-change branch of `reconcileReplicaUnits`; pure transition types in `pkg/storageunit` (`handoff.go`), the controller in `pkg/cluster` (`multibackend_overlap.go`), `DurableEpochReplica` lifted onto the `ReplicaBackendFactory` interface. Acceptance: an in-process loss-oracle gate with an ARBITRARILY SLOW `OpenReplicaUnit` asserts zero acked loss AND acked/attempted ~100% even under a multi-second mount (where Option A alone collapses), demonstrated to FAIL on a forced clean-cut. Option A's retry is retained as the safety net for the residual unserved instant (crash mid-handoff, pure-new-mount convergence); A and B compose. See "v0.8 Phase 2e" above.
