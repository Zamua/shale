...
**IN SCOPE: write availability through an R>1 (and R=1) multi-backend membership change, via a `WriteTimeout`-bounded retry-on-acquiring** (layer 1 makes the acquiring refusal a true fan-out transient; layer 2 retries the whole op when W is momentarily unreachable). The handoff-window refusal becomes bounded LATENCY, not an error. **OUT OF SCOPE (future optimization):** Option B (overlap / lease-handoff): pre-mounting the new owner and catching it up WHILE the old owner serves, flipping ownership atomically only once the new owner is ready, so there is no window with fewer than W ready replicas and the retry waits on nothing. B eliminates even the latency A leaves; it is a larger change to reconcile ordering + routing + cutover fencing and is deferred until the staging chaos shows A's residual latency tail matters. Also out of scope (unchanged from Phase 2b): re-replicating a unit's BYTES onto a newly-joined replica position that has no durable copy yet (Phase 2d assumes the reassigned replica's shared-storage database already exists, the copy-free case - true for a node taking over an existing replica position; standing up a brand-new R-th copy after a permanent node loss is the separate anti-entropy re-replication phase).
