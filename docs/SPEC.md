...
**The reconcile loop + four guard conditions.** The coordinator runs a reconcile pass (folded into the existing membership-settle debounce machinery, the same `settleTimer` that drives rebalance Evaluate). Concretely the decision lives in `reconcileReshard`, reached from TWO paths. (a) The SETTLE pass `reconcileAndMaybeReshard`, fired by the `settleTimer` debounce at the TAIL of the settled unit reconcile (after the lease-handoff `reconcileUnits` pass); it calls `reconcileReshard` directly. (b) The PERIODIC tick in `runReconcileLoop` (the existing fixed-cadence reconcile loop, the same ~5s ticker that re-syncs the ring), which calls `reconcileReshardIfStable` (a wrapper that enforces guard 1 explicitly, see below, then calls `reconcileReshard`). The settle pass is what fires once a rolling deploy settles (membership stops churning, every node reports the new desired); the periodic tick is the backstop for a coordinator handoff that arrives WITHOUT a fresh membership event on the new coordinator (the dead coordinator's leave settled before the survivor recomputed it is now lowest-id), so the new coordinator still re-evaluates a still-needed reshard. Both reach the same idempotent `reconcileReshard`; running it twice is a safe no-op. On each pass it reshards toward the desired count ONLY when ALL FOUR guards hold:

  1. **Membership is STABLE.** A reshard fires only after membership has been UNCHANGED for a settle window. This is load-bearing for a rolling deploy: while nodes are being replaced one at a time they report MIXED desired counts (some old at 2, some new at 4), and membership is churning; the stability requirement ensures the reshard never fires mid-roll, only once the deploy has settled and every node is the new revision. BOTH paths enforce this guard, but differently. The settle pass inherits it STRUCTURALLY from the `settleTimer` debounce (every join/leave re-arms the timer, so the pass cannot fire until the window has elapsed). The PERIODIC tick fires at a fixed cadence and CANNOT rely on the debounce, so `reconcileReshardIfStable` checks stability EXPLICITLY via `membershipStableFor(settleDelay)`: it reads a `lastRingChangeNanos` timestamp (stamped on every `bumpRingGen` ring-shape change, the chokepoint every membership/reconcile/propose ring change flows through) and skips the reshard until at least `settleDelay` has elapsed since the last change (a never-changed ring reads as stable, which is correct). Without this explicit check the periodic tick would bypass the debounce and could evaluate a reshard mid-roll, saved only by guard 2 (unanimity usually false mid-roll) plus the barrier's member-identity re-check on abort - defense-in-depth, NOT the stability guarantee this guard claims. The explicit check closes that gap, so the periodic tick correctly DELAYS a coordinator-handoff reshard by `settleDelay` after the triggering membership change (the dead coordinator leaving stamps `lastRingChangeNanos`).
