...
**Release is POLL-ONLY (no push RPC).** The release signal is a DURABLE, POLL-OBSERVABLE "serving marker," NOT a best-effort push. The NEW owner writes the serving marker to shared storage (keyed by the `ReplicaUnit`, carrying its open epoch) EXACTLY ONCE, right AFTER its mount flip, at the `Acquiring -> Ready` transition. The OLD owner's `drainCheck` POLLS the serving marker on the existing periodic settle / self-heal cadence and releases ONLY when it observes a marker at an epoch STRICTLY ABOVE (`>`) its own open epoch. The boundary is STRICT (not `>=`) to reject the old owner's OWN stale gain-marker: a node that GAINED the position at epoch E wrote a marker at E, and if the ring later moves it OFF, its open epoch stays E, so a `>=` gate would read its own marker and release prematurely while the real successor is still mid-mount. A genuine successor opens at `durable+1 >= E+1` and writes a marker strictly above E, so `>` still releases on a real successor. There is no `ReplicaHandoffReady` RPC and no RPC wake: the marker IS the signal, and a bare durable FENCE-epoch advance NEVER triggers release (it is strictly weaker than the serving marker).
