...
**Desired-count broadcast via Meta.** Each node advertises its desired count alongside its existing gRPC `Addr` in the memberlist per-node `Meta` payload - the same channel a node already uses to broadcast its forwarding address (`membership.Config.GRPCAddr` -> `metaDelegate.NodeMeta`, decoded by peers into `Member.Addr`). The `Meta` byte payload becomes a small structured value carrying BOTH the gRPC addr AND the desired count (e.g. `"<grpcAddr>|<desiredCount>"`, parsed back on receipt). `membership.Member` grows a `DesiredCount` field; `membership.Config` grows a `DesiredCount` input the node sets from its `--unit-count`. A peer with an older/empty Meta (no count segment) decodes to `DesiredCount = 0` (unknown), which the reconciler treats as "not yet agreed" - never as a reshard trigger. The broadcast is free: it piggybacks on the gossip every node already does, so every node sees every other node's desired count without a new RPC.

**The deterministic coordinator (exactly one).** A reshard must be kicked off by exactly ONE node - two nodes both starting a barrier toward g+1 would each try to freeze the cluster and race. The coordinator is chosen DETERMINISTICALLY: the lowest live node-id among the current members (`Members()` is already sorted by id; the coordinator is `Members()[0]`). Every node runs the same selection against the same membership snapshot, so they all agree on who the coordinator is without an election protocol. Only the node that finds itself to be the lowest-id member runs the reconcile-driven reshard; every other node is a passive participant that responds to the barrier phases (`ReshardControl`) as it already does. If the coordinator dies, the next-lowest id becomes coordinator on the next membership-settle and drives any still-needed reshard - the reconcile is idempotent, so a coordinator handoff mid-decision re-evaluates cleanly rather than double-resharding.

**The reconcile loop + four guard conditions.** The coordinator runs a reconcile pass (folded into the existing membership-settle debounce machinery, the same `settleTimer` that drives rebalance Evaluate). On each pass it reshards toward the desired count ONLY when ALL FOUR guards hold:
...
The derivation uses the slate factory's `Backing.PresentUnits` (already implemented: it lists the shared bucket under the unit prefix and parses each `g<gen>/u<id>` object key back to a `GenUnit`). On founder Open, `initMultiBackend` (in the no-seeds branch, BEFORE the mount loop) calls `PresentUnits`; if the bucket contains units, the founder derives the live `{gen, count}` from them - the highest fully-present generation `g` and the count of units at `g` (which, by the doubling invariant, is a power of two = the live count) - and `commitGenState`s THAT instead of the gen-0/desired default. A FRESH cluster (no durable units present) establishes at the desired count (`genState{gen: 0, count: --unit-count}`), the only place the desired seeds the actual count directly. This mirrors the joiner's `learnGenerationFromSeed` (a joiner learns the live `{gen, count}` from a seed before mounting); the founder learns it from the durable backing instead, since it has no seed to ask. We pick `PresentUnits`-derivation over a separately-persisted `{gen,count}` cluster-state object because the units in the bucket ARE the authoritative durable state already (no second source of truth to keep in sync, no write-it-on-every-reshard step that could desync from the actual unit set); `PresentUnits` reads the same bytes the mount loop will open. Joiners are unaffected: they still `learnGenerationFromSeed`, and they additionally broadcast their own desired count via Meta (so the coordinator sees unanimity once the roll completes). The factory interface exposes `PresentUnits` only on the slate backing today; the in-process test double has the same enumeration, and a backend whose factory cannot enumerate present units cannot be a founder of a previously-resharded cluster (it would have to carry a persisted cluster-state object instead - out of scope here, since the only deployable multi-backend factory is slate).
