...
**Release is POLL-ONLY (no push RPC).** The release signal is a DURABLE, POLL-OBSERVABLE "serving marker," NOT a best-effort push. The NEW owner writes the serving marker to shared storage (keyed by the `ReplicaUnit`, carrying its open epoch) EXACTLY ONCE, right AFTER its mount flip, at the `Acquiring -> Ready` transition. The OLD owner's `drainCheck` POLLS the serving marker on the existing periodic settle / self-heal cadence and releases ONLY when it observes a marker at an epoch `>=` its own open epoch. There is no `ReplicaHandoffReady` RPC and no RPC wake: the marker IS the signal, and a bare durable FENCE-epoch advance NEVER triggers release (it is strictly weaker than the serving marker).
