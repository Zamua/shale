...
- **OpenUnit(gu, epoch).** Opens (or creates on first touch) the slatedb instance at `dbName(gu)` in the shared bucket via the existing `slate.New` path, with `WriteOptions{AwaitDurable: true}` (the multi-backend invariant - see Durability below) and the operator's `Settings` (memtable sized small so many units fit one node, per the constraints section). Opening that instance IS the fence (see Epoch fencing). Returns the `slate.Slate` as a `backend.Backend`. The `Handle` records `gu -> {slate, openedEpoch}` in its in-process open map. Re-opening a `gu` THIS Handle already holds at an equal-or-lower epoch is a double-open error (a unit has at most one live writer per handle), matching the `sharedfactory` Handle contract; a strictly-higher re-open is allowed (the same node bumping its own epoch). The `epoch` argument is the cluster's best-effort intended floor; the actual fence is authoritative against the durable manifest (next subsection), so a stale floor cannot under-fence.
...
OpenUnit's fencing contract:
  ...
  4. The Handle records the opened epoch so `CurrentEpoch` and a same-node re-open report it.
...
#### Caveats to fix in this milestone (shipped together)

The slate factory review left two P2 + one P3; all SHOULD be fixed in this milestone since the factory now ships in the deployable binary:

  - **(P2) Fence-epoch test sensitivity.** The factory's fence integration test passed even with a NEUTERED `fenceEpoch` (it proves slatedb fences, but not that the factory computes the RIGHT intended epoch). FIX: add a test that pins the factory's epoch ARITHMETIC directly (open a unit, capture the durable epoch via `Backing.durableEpoch`, re-open and assert the new epoch is strictly `durable+1`), so a regression in the factory's own epoch math fails a test rather than hiding behind slatedb's own fencing.
  - **(P2) `Handle.OpenUnit` lock window.** `OpenUnit` releases its mutex between the held-check and the final map insert, with the real slatedb open in between - a narrow same-unit concurrent-open window where two goroutines could both pass the held-check and both open. FIX: hold the per-unit open under a single critical section (a per-`GenUnit` keyed mutex, or a "claim the slot before the slatedb open" placeholder under the handle mutex) so a same-unit concurrent open serializes. The cluster's `mountMu` + `reconcileMu` make this unreachable in the SUT today, but the factory is a public seam and must be correct standalone.
  - **(P3) Substring fence classification.** `tests/chaos/harness.go isFenceTransient` classifies fence errors by SUBSTRING match on `"Reason=2"` (brittle: a slatedb error-string change silently reclassifies a real fence as a real loss). FIX: classify on a typed/sentinel signal where the slate backend can expose one (a wrapped error the harness can `errors.Is` / `errors.As` against), falling back to the substring only if no typed signal is reachable. Lower priority because it is test-classification, not SUT behavior.
