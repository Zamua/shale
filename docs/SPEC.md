...
**The fix: open positions through a bounded worker pool.** The boot mount opens the node's owned positions with BOUNDED CONCURRENCY rather than strictly one at a time, collapsing the wall-clock from `sum(latency)` to roughly `ceil(n / concurrency)` opens. The pool size is CAPPED (`Config.OpenConcurrency`; zero is normalized to `defaultOpenConcurrency` = 4) on purpose: the cap trades a BOUNDED cold-start load spike on the shared object store for the latency win, instead of an unbounded fan-out that could itself overwhelm the store (the exact overload a cold start must avoid). The knob is a cluster Config field, not an env read in the core (env -> Config mapping is the deploying binary's job); a deployment that wants to tune it sets it from its own environment. A node owning 16 positions against a store with ~seconds-per-open goes from a ~minutes serial mount to a ~`16/4` = 4-deep mount.

**The default is deliberately LOW (4, not a bigger number), and the open is RETRYABLE - because the concurrent-open burst can saturate a CPU-constrained store.** Each open does many parallel object GET/LIST ops; N concurrent opens multiply that into a heavy short CPU spike on the store. A store running under a CPU LIMIT will CFS-throttle under that spike, and a throttled/overloaded object store can return TRUNCATED reads that slatedb rejects as `empty SSTable` (a too-small SST/WAL object). So: (1) keep the default concurrency modest so the burst is small relative to typical store capacity; (2) treat `empty SSTable` (and similar transient read-truncation errors) on a boot open as RETRYABLE - the per-open path retries a bounded number of times with backoff before giving up to degraded boot, so a transient truncation under momentary load self-heals instead of skipping the position; and (3) the deploying operator should run the object store UNCAPPED on CPU (or generously limited) so the burst cannot throttle it. The retry only applies to a RETURNED transient error, never to a timed-out (still-running) open - retrying a stalled open would race the live un-cancellable open against the writer-epoch fence. A genuinely corrupt/empty object still exhausts the retries and falls through to degraded boot, unchanged.
