...
**Node metadata: address + a `Draining` bit.** Each node publishes a small `Meta` payload over gossip. It carries the node's gRPC dial address (so peers can forward to it) AND a boolean `Draining` flag used by the graceful-leave (scale-down) path (see "v0.8 Phase 2e: Graceful leave"). The `Member` value object the membership layer returns from `Members()` / `Snapshot()` exposes both fields (`Addr` and `Draining`), decoded from the peer's `Meta` in the `NotifyJoin` / `NotifyUpdate` callbacks. A `Membership.SetDraining(bool)` method updates the LOCAL node's `Meta` flag and calls `memberlist.UpdateNode` so the change gossips out; the node STAYS a full, alive member - `SetDraining(true)` is NOT `Leave()`. A `Draining` member is REACHABLE (still in the snapshot, address known, serving gRPC) but is EXCLUDED FROM OWNERSHIP by the ring-from-membership reconcile (`reconcileRingFromMembership` drops a `Draining` member from the consistent-hash ownership ring on every node, so `LocateKeyN` redistributes its positions to non-draining members while its address stays resolvable for in-flight forwards). This is the foundation of the draining node-state described in Phase 2e.
...
**IN SCOPE: write availability through an R>1 (and R=1) multi-backend membership change, via a `WriteTimeout`-bounded retry-on-acquiring** (layer 1 makes the acquiring refusal a true fan-out transient; layer 2 retries the whole op when W is momentarily unreachable). The handoff-window refusal becomes bounded LATENCY, not an error. **SUPERSEDED FOR THE BIG-CHURN CASE by Phase 2e (Option B, overlap handoff) below:** Option A's availability is bounded by mount-time-vs-`WriteTimeout`. Measured on real 3-node staging: incremental scale 3 -> 4 stays ~100% (few units remount, fast), but big-bang 3 -> 12 (about 16 slatedb databases remounting at once, real MinIO mounts exceeding the 5s budget) drops to ~54% as the retry budget exhausts. Phase 2e removes that bound: the old owner keeps serving until the new owner has FULLY acquired, so availability no longer depends on mount time at all. Option A is RETAINED as the safety net for the residual unserved instant Phase 2e leaves (a node crash mid-handoff, the pure-new-mount initial convergence); A and B compose (A is the belt, B shrinks the window A waits on to near zero). Also out of scope (unchanged from Phase 2b): re-replicating a unit's BYTES onto a newly-joined replica position that has no durable copy yet (the copy-free case - a node taking over an EXISTING replica position - is what 2d and 2e handle; standing up a brand-new R-th copy after a permanent node loss is the separate anti-entropy re-replication phase).

### v0.8 Phase 2e: overlap handoff (Option B, R>1 membership change)

Phase 2d (Option A) keeps R>1 writes available across a membership change by RETRYING through the acquiring window, bounded by `WriteTimeout`. That makes availability a function of mount-time-vs-timeout: a single slatedb mount from MinIO can exceed the 5s budget under a big-bang scale-out (~16 databases remounting at once), and the measured big-bang 3 -> 12 ack rate falls to ~54%. Phase 2e (Option B) removes the dependency on mount time entirely: the OLD owner KEEPS SERVING a moving `ReplicaUnit` until the NEW owner has FULLY acquired it (mounted + durably fenced + ready), and DURING that overlap the NEW owner (which routing already targets once membership converges) FORWARDS routed ops to the OLD owner so the position is never stranded. The mount flip (the new owner serving locally) and the old owner's release both happen exactly when the new owner becomes ready, epoch-fenced, with no instant where the position is unserved. Availability then no longer depends on mount time at all. A 30s mount is a non-event. The detailed design (state diagram, sequence, package layout, the R/W crash matrix) is in `docs/design/overlap-handoff.md`; this section is the canonical behavior.

**Release is POLL-ONLY (no push RPC).** The release signal is a DURABLE, POLL-OBSERVABLE "serving marker," NOT a best-effort push. A node writes the serving marker to shared storage (keyed by the `ReplicaUnit`, carrying its open epoch) EXACTLY ONCE per mount, right AFTER it completes a mount of the position: the overlap gainer writes it at the `Acquiring -> Ready` mount flip, AND the clean-cut acquire (`acquireReplicaUnit`, the Option-A / pure-new-mount / non-single-hop path) writes it right after it inserts the mount entry. The clean-cut path MUST write it too because a position moving OFF a leaving node can land on its successor via the clean-cut path (a full-rebalance move whose predecessor is no longer single-hop identifiable from the prior snapshot): the leaving node is DRAINING that exact position and releases ONLY on a marker strictly above its open epoch, so a silent clean-cut mount would strand the drain for the whole grace timeout. The marker is monotonic + idempotent, so writing it on a pure new mount with no draining predecessor is a harmless observer no-op. The OLD owner's `drainCheck` POLLS the serving marker on the existing periodic settle / self-heal cadence and releases ONLY when it observes a marker at an epoch STRICTLY ABOVE (`>`) its own open epoch. The boundary is STRICT (not `>=`) to reject the old owner's OWN stale gain-marker: a node that GAINED the position at epoch E wrote a marker at E, and if the ring later moves it OFF, its open epoch stays E, so a `>=` gate would read its own marker and release prematurely while the real successor is still mid-mount. A genuine successor opens at `durable+1 >= E+1` and writes a marker strictly above E, so `>` still releases on a real successor. There is no `ReplicaHandoffReady` RPC and no RPC wake: the marker IS the signal, and a bare durable FENCE-epoch advance NEVER triggers release (it is strictly weaker than the serving marker).

**Two flips, named distinctly.** There are TWO events, not one. The RING ROTATION: membership converges and the ring's replica set for a position changes (the OLD owner drops out, the NEW owner enters); the write/read fan-out (`replicasForKey -> unitReplicas -> LocateKeyN`) is re-derived per op from the LIVE ring, so the ring rotation is what steers which nodes are contacted, and it happens BEFORE any mount completes. The MOUNT FLIP: the NEW owner finishes `OpenReplicaUnit` and inserts its `mountMap` entry, so a routed op resolves LOCALLY on the new owner instead of being forwarded to the predecessor; this is the correctness cut, strictly after the durable fence. Option B's correctness argument is about the mount flip; AVAILABILITY is governed by the ring rotation, which is why the new owner must be usable during `Acquiring` (it forwards to the predecessor).

**The clean shape: ONE per-`ReplicaUnit` ownership-transition state machine, not scattered flags.** A handoff today is two UNSEQUENCED halves on two nodes: the losing node drops the position from its desired set and RELEASEs; the gaining node adds it and ACQUIREs. Nothing orders them, so there is a window where NEITHER serves (the bug). Phase 2e replaces the position-change branch of `reconcileReplicaUnits` (the RELEASE-then-ACQUIRE) with an ACQUIRE-then-RELEASE driven by an explicit phase per side. The unit of ownership is the `ReplicaUnit` = `(GenUnit, replica-position)`, node-independent (the same durable prefix `dbNameReplica(ru)` is what the old owner has open and what the new owner opens), which is exactly what makes overlap possible. **The in-memory maps are keyed by `ReplicaUnit`, NOT `GenUnit`:** `mountMap` becomes `map[ReplicaUnit]Backend`, the separate `replicaPos` map is folded into the key, and the new `handoffPhase map[ReplicaUnit]HandoffState` is a mountMu-guarded sibling. This is REQUIRED because a ring change can shuffle this node's index within a unit's replica list, so one node must hold the OLD position (draining) AND the NEW position (acquiring/ready) of the SAME unit at once; a per-`GenUnit` map cannot represent both. The re-keying ripples through `localBackendForKey` / `localWriteBackendForKey` / `evictStaleMount` / `mountedBackends` and the reconcile diff (each resolves the position from this node's ring index for the key's unit). The complete state of an in-flight position is the phase value + the mount entry + (in `Acquiring`) the recorded predecessor address; no `isDraining` / `hasFlipped` / `pendingRelease` booleans, and every transition is a single guarded edge.

**The two halves of the machine.**

  - ON THE NEW OWNER (the node GAINING the position): `Absent -> Acquiring -> Ready -> Owned`. `Acquiring`: desired by the LIVE ring (which has rotated to target this node) but not yet mounted; `OpenReplicaUnit` is running the slow MinIO mount + per-replica fence. Because routing now targets this node yet it has no local mount, a routed op is NOT refused with `errUnitAcquiring`: instead the new owner FORWARDS the op to the PREDECESSOR (the old owner, identified from the retained prior-desired-replica-set snapshot; see the readiness-signal section) over RPC, read AND write transparent, and returns the predecessor's result. **The forward is POSITION-ADDRESSED (a real WIRE change, NOT a reuse of the key-only forwarded ops):** the predecessor no longer holds this position on its LIVE ring index (the position moved away), so the existing key-only `forwarded` ops would re-resolve to the wrong/absent position on the predecessor. The forwarded op therefore carries the EXPLICIT `ReplicaUnit` (a `ReplicaUnit` field added to the `PutRequest` / `GetRequest` / `DeleteRequest` / CAS-apply-forward path, or a new position-addressed variant), and the predecessor's forwarded-op handler resolves `mountMap[ru]` DIRECTLY by that ru - serving a `Draining`-phase entry - bypassing its live-ring-index resolver, through the SAME `handoffPhase` / `mountMap[ru]` structures (no parallel draining map). It still reuses the `clientFor` plumbing, just aimed at the predecessor's address and addressed by ru. If the predecessor is UNREACHABLE, OR no unambiguous predecessor was identifiable (the single-hop fallback, below), the new owner FALLS THROUGH to `errUnitAcquiring` (the Option-A retry belt). THIS IS THE OVERLAP WINDOW, and mount time here is irrelevant to availability. `Ready`: `OpenReplicaUnit` returned at a fenced epoch E (strictly above the durable manifest writer-epoch); the `mountMap[ru]` entry is inserted under mountMu in ONE critical section. THE INSTANT THAT ENTRY IS VISIBLE, this node serves routed ops LOCALLY and STOPS forwarding (THE MOUNT FLIP), and writes the SERVING MARKER (`WriteServingMarker(ru, E)`) to shared storage EXACTLY ONCE (the durable, poll-observable release signal; written only here, after the flip). Then `Ready -> Owned` (the transient phase entry is dropped; steady state is just a mounted entry).

  - ON THE OLD OWNER (the node LOSING the position): `Owned -> Draining -> Releasing -> Absent`. `Draining`: the reconcile computed this position is no longer desired here, but the old owner does NOT delete the mountMap entry; it KEEPS SERVING (its own routed ops AND the new owner's forwards) and records `Draining` + arms a release-check (`drainCheck`, on the periodic settle / self-heal cadence; no RPC wake). `Releasing`: the release-check POLLED the durable serving marker (`ReadServingMarker(ru)`) and observed a marker at an epoch STRICTLY ABOVE (`>`) its own open epoch (positive readiness confirmation that a live SUCCESSOR is actually SERVING the position; the strict `>` rejects the old owner's own stale gain-marker at exactly its open epoch), so the old owner compare-and-deletes its mountMap entry (only if it still points at the same backend, reusing the `evictStaleMount` CAS shape) and calls `CloseReplicaUnit(ru)` EXACTLY ONCE, then `Releasing -> Absent`. It NEVER releases on a bare durable FENCE-epoch advance (the fence bumps at open-START, before the mount completes; only the serving marker proves a live owner is serving).

**The dovetail: release-on-Ready == stop-forwarding.** The old owner releases exactly when the new owner becomes `Ready`, which is also exactly when the new owner stops forwarding. One event (the mount flip) drives both: the new owner flips to local serving (stops forwarding to the predecessor) AND writes the serving marker the old owner polls to release on. There is no window where the old owner has released while the new owner is still forwarding to it: the old owner releases only after observing the marker, and the marker exists only after the flip (which is also when forwarding stops). The release is POLL-driven, so the old owner observes it on its next `drainCheck` tick rather than on a push.

**The cut (the exact epoch-fence point).** The handoff cut is a single instant on the per-replica durable manifest: the epoch E at which the NEW owner opened (`OpenReplicaUnit` opens at `max(intended, durableEpochReplica+1)`, and that open WRITES the bumped epoch to the manifest). Below E, the OLD owner is the authoritative writer; at or above E, the NEW owner is. The OLD owner's own writes are fenced the moment it attempts one past E (slatedb `CloseReasonFenced`), so even a buggy late write on the old side fails rather than corrupting the position. The mount flip happens strictly AFTER the fence (the open completed first), so there is never an instant where BOTH nodes serve the position LOCALLY AND both are unfenced. Forwarding does not violate this: a forwarded write lands on the OLD owner BELOW E (the authoritative writer there until E), and the new owner does not serve locally until after E.

**The signal: the durable serving marker (poll-observable), the durable fence epoch (a strictly-weaker hint that NEVER releases).** There is NO new central controller, NO new coordination protocol, and NO push RPC for correctness. Each node runs `reconcileReplicaUnits` independently off the existing debounced settle timer, driven by the same membership topology events; the ring is the shared truth and both nodes independently conclude the position moves. Release is POLL-ONLY, driven by ONE durable signal (the serving marker) over shared storage, with the fence epoch as a contrast that is explicitly NOT a release trigger:

  1. THE RELEASE SIGNAL (durable + poll-observable): the SERVING MARKER, a small durable record keyed by the `ReplicaUnit` and carrying its open epoch, written to shared storage by the NEW owner EXACTLY ONCE at the `Acquiring -> Ready` transition (right after the mount flip: after `OpenReplicaUnit` returns, after `mountMap[ru]` is inserted, after the new owner starts serving locally). The marker means "a live owner is actually SERVING this position," which is STRICTLY STRONGER than the durable fence epoch. The old owner's `drainCheck` POLLS it via `ReadServingMarker(ru)` on the existing settle / self-heal cadence and releases ONLY when it reads a marker at an epoch STRICTLY ABOVE (`>`) its own open epoch (the strict `>` rejects the old owner's own stale gain-marker at exactly its open epoch; a real successor opens at `durable+1` and writes a marker strictly higher). There is no push and no RPC wake; the marker IS the release signal, observed on the next poll tick. The marker is added to the factory seam: `WriteServingMarker(ru, epoch)` + `ReadServingMarker(ru) (epoch, ok)`, implemented in-memory on the test `sharedfactory` and as a small durable object keyed by `ru` on the slate factory.
  2. NOT A RELEASE TRIGGER (the contrast): the DURABLE per-replica manifest writer-epoch (the FENCE epoch), readable WITHOUT opening via `DurableEpochReplica(ru)` (the slate `Backing` already exposes it; Phase 2e lifts it onto the `ReplicaBackendFactory` interface). It bumps at OPEN-START (before the mount completes), so a new owner that fences and then crashes mid-mount advances the fence epoch WITHOUT ever serving and WITHOUT ever writing a marker. A BARE fence-epoch advance therefore NEVER releases the old owner; only the serving marker (written after the flip) does. This is the P1-1 safety: the marker is the positive "live owner is serving" confirmation the bare fence cannot give. **The marker read is a POINT-IN-TIME liveness observation, not a lease/latch:** if NEW crashes in the gap between the old owner reading the marker and the completion of its release (the old owner does not re-read inside the lock - I/O under the lock is forbidden), the old owner releases and the position is unserved until the NEXT RECONCILE reassigns it, with NO acked-write loss (durable-before-ack). So the marker makes the old owner safe against a NEW that crashed BEFORE becoming Ready (it never wrote the marker; crash case (a)), NOT against a crash in the release gap; the gap's recovery is the benign next reconcile (see guarantee 3).

**Predecessor identification + single-hop scope.** The new owner must know WHICH node held the moving position before, to forward to it. The multi-backend reconcile keeps NO prior-ring snapshot today (only the LEGACY per-node path has `lastEvalRing`); `reconcileReplicaUnits` computes only `desiredReplicaUnits()` against the LIVE ring. Phase 2e ADDS a retained prior-desired-replica-set snapshot (the multi analogue of `lastEvalRing`): the desired replica sets as of the ring the PREVIOUS reconcile used, captured at the END of each reconcile on a `mountMu`-guarded `Cluster` field, consulted ONLY to identify the predecessor (NEVER for routing). The predecessor of a moving position is `prior-snapshot-holder \ live-holder` of that position. **The predecessor's DIAL ADDRESS is captured alongside the snapshot (a sibling `priorAddrs` map from the SAME prior ring) and STORED in the `Acquiring` `HandoffState` (`PredecessorAddr`), not just its `NodeID`.** This is required for the graceful-leave (scale-down) case: when the predecessor goes `Draining`, it is EXCLUDED FROM THE OWNERSHIP RING (so its positions redistribute), so resolving its `NodeID` to an address via a live OWNERSHIP-ring walk returns nothing and the forward gives up while the still-alive, still-`Draining` node is serving. (The node is still in MEMBERSHIP - reachable, address known - it is only excluded from OWNERSHIP; but the forward target resolution walks the ownership ring, which no longer lists it.) Storing the address with the per-position handoff state means the forward reaches it regardless of the ownership ring; the address string is plain data so the pure `HandoffState` stays I/O-free. (A single global "addresses as of the last reconcile" map does NOT suffice: it is overwritten each reconcile and would no longer hold the departed node's address by request time, which is after the ring has moved past the leave - the address must persist WITH the per-position handoff state for the handoff's duration.) The snapshot is point-to-point, so a position that moves twice within one debounce window (OLD -> MID -> NEW coalesced into one reconcile) cannot be resolved to a single live predecessor. Overlap is therefore SCOPED TO SINGLE-HOP and BEST-EFFORT: when the prior-snapshot predecessor is AMBIGUOUS or STALE (the prior holder is gone or already released), the new owner records NO predecessor and the `Acquiring` op FALLS THROUGH to the Option-A clean-cut belt (`errUnitAcquiring` + bounded retry) rather than forwarding to a stale node. A big-churn coalesced move thus degrades to today's Option-A behavior; overlap shrinks the common-case window to near zero and leaves the rare multi-hop case to Option A.

**The slate manifest seal (consistent mount under overlap).** The new owner must mount a view containing every write the old owner ACKED (whether a direct write to the old owner before the ring rotated, or a write forwarded through the new owner to the old owner during the overlap; both land on the old owner's authoritative database below epoch E). This holds without a new flush protocol because: (1) DURABLE-BEFORE-ACK: the slate replica opens with `AwaitDurable=true`, so every ACKED write is durable in the WAL/SST BEFORE its ack returns; this holds identically for a forwarded write (the predecessor applies it `AwaitDurable=true` before the forwarded ack returns). (2) WAL RECOVERY OVER THE FULL DURABLE TAIL: the new owner's `OpenReplicaUnit` must recover the FULL durable WAL tail, not merely load the manifest snapshot it read at fence time. An old-owner write durable-before-ack at `T_write < T_fence` is durable in the WAL but may NOT yet be reflected in the manifest the new owner read at `T_fence` (slatedb's WAL is durable independently of manifest checkpoints; a freshly-durable WAL entry is recovered on open via WAL replay, not via the manifest snapshot). For the seal to hold, NEW's open must replay that WAL tail. **INTRA-OPEN ORDERING (forwarding widens the window):** because forwarding continues until the mount flip, which is strictly AFTER `OpenReplicaUnit` completes, a forwarded write can be acked on the old owner AFTER NEW's recovery snapshot but still BELOW E. To keep every such write inside NEW's recovered tail, the manifest-epoch FENCE must be effective NO LATER THAN the WAL-recovery cutoff within a single `OpenReplicaUnit`: make the fence effective FIRST, THEN take the recovery snapshot (or re-scan the WAL tail after the fence). Then a write past the fence is rejected on the old owner and never acks; a write before the fence is in the recovered tail. The slate `OpenReplicaUnit` TODAY orders `fenceEpochReplica` (writes the bumped manifest epoch) before `openSlateReplica` (the db open where the Rust core recovers), which is the correct fence-then-recover order at the Go level; whether the Rust core takes its recovery snapshot at-or-after the bumped epoch is FFI-opaque and must be pinned (if it recovers-then-fences internally, the slate open must be adjusted to re-scan the WAL tail after the fence). **This is an EXPLICIT ASSUMPTION the implementation MUST PIN with TWO tests** (1) a write acked on the old owner JUST BEFORE the fence must be readable on the new owner AFTER the handoff; (2) a write driven THROUGH THE FORWARD PATH CONCURRENTLY with the flip (acked on the old owner below E while NEW's open/recovery races) must be readable on the new owner AFTER the handoff. The slatedb-go v0.13.1 binding is a uniffi FFI over the Rust core and does not expose the open/recovery path in Go source, so we cannot confirm WAL-replay-on-open OR the fence<=recovery ordering from the module; it is consistent with the binding's independent `WalReader` surface but not proven. If slatedb open does NOT replay the durable WAL tail, OR the fence is not effective at-or-before the recovery cutoff, this is a P0 lost-write and Phase 2e is BLOCKED until the gap is closed (an old-side flush/checkpoint into the manifest before the new owner opens, pinning the slate open to re-scan after the fence, or another seal protocol). (3) The fence is on the durable manifest writer-epoch; the new owner's open reads the manifest and lands above it. The difference Option B introduces over Option A is that the new owner now acquires WHILE the old owner is still writing (in A the old owner sealed via `CloseReplicaUnit` BEFORE the new owner acquired). That is safe: the new owner does not need the old owner's UN-acked in-flight writes (the client never got success for those and retries), only every ACKED write, durable strictly BEFORE the fence. The FENCE-DETECTION CHAIN proves no acked-past-fence write exists: (a) slatedb detects a fence on the old writer's next manifest/WAL-touching write (its epoch is below E), AT the durable write; (b) `AwaitDurable=true` forces that durable write to complete BEFORE the ack returns; (c) therefore a write that WOULD be fenced is rejected (`CloseReasonFenced`) at its durable-write step, strictly before any ack, so it CANNOT have acked. Either a write durably committed before E (acked, durable, visible to NEW) or it is fenced (never acked); there is no "acked on old, invisible to new" gap. This chain holds identically for a forwarded write (the predecessor does the durable write). The old owner's `CloseReplicaUnit` at `Releasing` still seals (Db.Shutdown flushes), but for acked writes it is belt-and-suspenders.

**THE FIVE CORRECTNESS GUARANTEES (argued explicitly).**

  1. NO LOST WRITES (a write acked before/during/after the flip is durable + visible after it). Before the ring rotates the write lands on the OLD owner directly; during the overlap (after the ring rotated, before the mount flip) it routes to the NEW owner which FORWARDS it to the OLD owner; both are `AwaitDurable=true` so the ack means durable at `dbNameReplica(ru)` and the new owner reads it on open (WAL recovery over the full durable tail, per the seal's explicit assumption). After the mount flip every write is on the new owner's now-authoritative database. At no instant is there a write that is acked but invisible to the side that will serve it next, because acks require durability and the fence orders the two sides on the durable manifest.

  2. NO DOUBLE-APPLY / NO SPLIT-BRAIN (old and new never both accept conflicting writes past the fence). The fence point is epoch E on the durable manifest. Exactly one side holds the database open at the highest epoch at any instant; the other side's writes fail with `CloseReasonFenced`. During the overlap the routed-to side is the NEW owner, which FORWARDS to the OLD owner (the authoritative writer below E), so the durable write happens on exactly one side. After the mount flip the new owner serves locally (the authoritative writer at/above E) and the old owner is fenced. A write that races the flip lands on the losing side's now-fenced handle, fails, is evicted (`evictStaleMount` -> `errUnitAcquiring`), and is retried onto the winning side. Apply-if-newer (LWW stamp) makes even a reordered or replayed envelope a no-op, so there is no conflicting double-apply.

  3. CRASH SAFETY (NOT unconditionally "fully available"; see the R/W matrix in the design doc). (a) NEW crashes mid-acquire (`Acquiring`, before the mount flip): it never wrote the serving marker; a bare durable FENCE-epoch advance from its pre-crash fence does NOT release the old owner (no marker strictly above OLD's epoch exists), so the old owner stays `Draining` and KEEPS SERVING. The only effect is that ops in-flight to the crashed new owner fail and retry; the next reconcile reassigns the position (possibly back to OLD, which drops `Draining` and returns to Owned). This is the crash the serving-marker release protects: under a bare-fence-epoch release NEW's pre-crash fence would have stranded the position. (b) OLD crashes while `Draining` (before NEW is Ready): the predecessor the new owner forwards to is gone, so the new owner falls through to `errUnitAcquiring` for the position, degrading to the Option-A window for THIS one position only, bounded by the new owner's mount, and only when the churn also kills the old owner mid-flight. Availability during this window depends on the R/W config: at R>=3/W=majority or R=2/W=1 the other replicas cover W; at R=2/W=2 (write-all) the position is write-unavailable until NEW mounts (only one live replica). Every acked write is already durable (durable-before-ack), so NEW sees them all once it mounts; no acked write is lost. (c) NEW crashes AFTER Ready but before OLD released: the serving marker was written at the flip, so OLD reads it on its next poll and releases, and the next reconcile reassigns the position (NEW is gone); if OLD has not yet polled, it stays `Draining`-serving until it does (then releases) or until the next reconcile reassigns the position. Either way no acked write is lost (all durable), and any stale OLD mount is torn down by `evictStaleMount` on its next fenced write. RESIDUAL (the marker read is point-in-time, not a latch): if NEW crashes in the gap between OLD reading the marker and the completion of OLD's release (OLD does not re-read inside the lock), OLD releases anyway and the position is UNSERVED until the next reconcile reassigns it. This is benign (no acked-write loss; durable-before-ack), and the recovery is the same next-reconcile path. The serving marker closes the bare-fence-epoch hole (it makes OLD safe against a NEW that crashed BEFORE Ready, case (a)); it does NOT make case (c) fully crash-safe against a crash-in-the-release-gap, and the doc does not claim it does.

  4. SLOW MOUNT IS A NON-EVENT. The entire `Acquiring` window is invisible to availability: the old owner serves the position throughout (directly before the ring rotated, then via the new owner's forward), and the new owner's mount can take 30s without any routed op failing. The mount flip happens only at the `Ready` insert, strictly after the mount completes, so a slow mount delays the flip but never opens an unserved window. This is the property Option A could not give (its availability was bounded by mount-vs-`WriteTimeout`).

  5. CLEAN TEARDOWN (the old owner releases EXACTLY ONCE, only after the new owner is Ready). The old owner transitions `Draining -> Releasing` only on a POSITIVE readiness signal (a serving marker at an epoch STRICTLY ABOVE (`>`) its own open epoch; the strict `>` rejects the old owner's own stale gain-marker at exactly its open epoch), NEVER on a bare durable FENCE-epoch advance. The `ReadServingMarker` poll (I/O) runs OUTSIDE any lock; the phase compare-and-advance (`Draining -> Releasing`) and the `mountMap[ru]` compare-and-delete are ONE critical section under `mountMu`, with the CAS-on-delete (delete only if the entry still points at the same backend) the real exactly-once guard so overlapping poll ticks reach `Releasing` at most once; the phase machine also rejects a second `Releasing` edge. `drainCheck` touches only `mountMu` + the phase map (never `reconcileMu` / `reshardMu`), and `CloseReplicaUnit(ru)` runs after the critical section.

**Resolved review findings.** Two adversarial reviews (`docs/design/overlap-handoff-review.md` first pass and `docs/design/overlap-handoff-rereview.md` re-review of the forwarding design, both kept as history) found holes the behavior above now closes:

  - P0-1 (routing never reaches the still-serving old owner): closed by NEW-OWNER FORWARDING - while `Acquiring`, the new owner forwards routed ops to the predecessor (read and write transparent) instead of refusing, falling through to `errUnitAcquiring` only if the predecessor is unreachable. The hot fan-out / W math is untouched; the overlap is confined to the new owner's `Acquiring` state. (Chosen over the pending-ranges union for minimal blast radius.)
  - P0-2 (per-`GenUnit` maps cannot represent overlapping positions): closed by RE-KEYING `mountMap`, the phase map, and the pure `HandoffState` by `ReplicaUnit` (folding `replicaPos` into the key), with the ripple through every mount reader.
  - P1-1 (release on a bare durable-epoch advance): closed by making "NEW OWNER IS READY" the release trigger via the durable, poll-observable SERVING MARKER the new owner writes only after the mount flip; the durable FENCE epoch (which bumps at open-START) is strictly weaker and NEVER releases on its own. This also fixes the crash-case-1 regression. (Poll-only: there is no push RPC; the old owner reads the marker on its `drainCheck` cadence.)
  - P1-2 (consistent-mount WAL tail): made explicit that the new owner's open must recover the full durable WAL tail; flagged as an IMPLEMENTATION-MUST-VERIFY assumption (slatedb-go is an opaque FFI binding) that gates release - a targeted write-then-fence test must pin it.
  - P1-3 (release-check lock discipline): pinned - I/O outside the lock; the phase advance + mountMap CAS-delete are one `mountMu` critical section (exactly-once); `drainCheck` touches only `mountMu` + the phase map.
  - P1-4 (crash-safety is not always "fully available"): stated honestly via the R/W matrix - the happy path is mount-time-independent, but a crash mid-handoff degrades the affected position to mount-bounded for configs where the surviving replica count cannot meet W (any single OLD crash at R=2/W=2).
  - P2-1 (term "flip" overloaded): split into RING ROTATION vs MOUNT FLIP, used consistently.
  - P2-2 (pure types under the two-sided machine): the pure `HandoffState` is keyed by `ReplicaUnit`, so one node can be `Draining` position p and `Acquiring` position q of one unit; the guards encode which phase is legal on which side.
  - P2-3 (overlap-move vs plain-release): distinguished - the overlap machine applies ONLY when a position moves to a SPECIFIC new owner; a node simply dropping out of a replica set (some other already-mounted replica covers W) stays a plain clean-cut `releaseReplicaUnit`.

The re-review found NEW holes the FORWARDING approach itself introduced; these are closed by the five operator decisions folded into the behavior above:

  - NEW-P0 (the forwarded op could not reach the predecessor's DRAINING mount, because the ReplicaUnit-keyed resolver is ring-index-keyed and the predecessor is off the ring for that position): closed by the POSITION-ADDRESSED forward - the forwarded op carries the explicit `ReplicaUnit` (a real WIRE change) and the predecessor's handler resolves `mountMap[ru]` directly, serving the `Draining` entry. The earlier "reuses the existing forwarded-op RPCs unchanged / no new proto" claim was FALSE and is removed.
  - NEW-P1-1 (predecessor identification had no source: the multi reconcile keeps no prior-ring snapshot): closed by ADDING a retained prior-desired-replica-set snapshot (the multi analogue of legacy `lastEvalRing`), captured at the END of each reconcile and consulted ONLY to identify the predecessor (`prior-holder \ live-holder`), never for routing.
  - NEW-P1-2 (a multi-hop / double-move coalesced into one debounce window mis-identifies the predecessor): closed by SCOPING overlap to SINGLE-HOP with an Option-A fallback - an ambiguous/stale predecessor records no predecessor and the `Acquiring` op degrades to the clean-cut belt rather than forwarding to a stale node.
  - NEW-P1-3 (the serving-marker read is point-in-time, so a NEW crash in the read-to-release gap lets OLD release with nobody serving): closed as an HONESTY fix - stated that the marker read is a point-in-time liveness observation; the gap leaves the position unserved until the next reconcile with NO acked-write loss, and the doc no longer claims case-3 is fully crash-safe.
  - NEW-P1-4 (forwarding widens the lost-write window beyond "acked before T_fence" to "acked while the open/recovery races the flip"): closed by PINNING the intra-`OpenReplicaUnit` ordering (the manifest-epoch fence effective no later than the WAL-recovery cutoff) and WIDENING the pin test to a write-through-the-forward-CONCURRENT-with-the-flip case (the slate open's internal recover-vs-fence order flagged for implementation).

**THE OVERLAP-HANDOFF AVAILABILITY GATE (the acceptance test).** A new in-process loss-oracle test (`tests/integration/overlap_handoff_test.go`, sharedfactory + memory backend, NO slatedb tag, NO MinIO) is the acceptance bar. It stands up a multi-backend R=2 cluster on a per-replica shared-backing factory whose `OpenReplicaUnit` can be made ARBITRARILY SLOW (a multi-second mount injection that models a real MinIO mount exceeding `WriteTimeout`), writes a recorded BASELINE, then runs a CONTINUOUS WRITER through a 3 -> N membership change while the slow mounts are in flight, and asserts BOTH (a) THE ORACLE - every baseline key and every ACKED probe key is readable with its exact value from every node, ZERO acked loss; AND (b) THE ACK RATE - acked / attempted is ~100% EVEN with the slow mount (where Option A alone would drop to ~50-54% because the retry budget exhausts on the mount). It is kept honest by a BREAK DEMONSTRATION that disables BOTH the overlap (forces the clean-cut RELEASE-then-ACQUIRE, so the new owner refuses during `Acquiring` instead of forwarding to the predecessor) AND the Option-A retry (so the retry cannot mask the gap): the same slow-mount membership change then drives the ack rate WELL below the threshold, proving the overlap forward is what holds availability rather than rubber-stamping it. (Disabling only the overlap while leaving Option-A retry on would let the retry partially absorb the slow mount and muddy the signal; the gate must isolate the overlap.) The record-only-acked discipline keeps (a) and (b) independent: durability must hold even at a low ack rate (the old behavior was lossless-but-unavailable), and the fix is the ack rate climbing to ~100% under a slow mount WITHOUT regressing durability.

**IN SCOPE: overlap handoff for an R>1 (and R=1) multi-backend membership change** - the old owner serves until the new owner is fully acquired + durably fenced + ready (the new owner forwarding routed ops to the predecessor during the overlap), the mount flip happens atomically at the new owner's `mountMap[ru]` insert, and the old owner releases exactly once when the new owner is Ready. Availability no longer depends on mount time. Expressed as ONE per-`ReplicaUnit` ownership-transition state machine (`Acquiring/Ready/Draining/Releasing` over the `Owned/Absent` poles), with `mountMap` / phase / `HandoffState` re-keyed by `ReplicaUnit`. **OUT OF SCOPE (unchanged):** re-replicating a unit's BYTES onto a brand-new replica position with no durable copy (anti-entropy re-replication is a separate phase); resharding / doubling at R>1 (Phase 4 is R=1); the legacy per-node path and the R=1 lease-handoff (Phase 3) are UNTOUCHED (Phase 2e lives behind `multiReplicated()`). Option A's `WriteTimeout`-bounded retry (Phase 2d) is RETAINED as the safety net for the residual cases (crash mid-handoff, pure-new-mount initial convergence); A and B compose.

#### Graceful leave (scale-down)

The overlap machine above makes scale-UP (a JOIN) mount-time-independent: a joining node ACQUIREs while the existing owner keeps `Draining`-serving, and the position is never unserved. Scale-DOWN (a deliberate node REMOVAL) is the SYMMETRIC case, and it has a gap the overlap machine does NOT yet close, because the gap is in the SHUTDOWN PATH, not the reconcile path. When a node is removed gracefully (a SIGTERM from the operator / orchestrator), the surviving nodes must re-own its positions and start `Acquiring` them (forwarding to the leaving node as the predecessor) exactly as for any membership change - from the LEAVING node's own view, every position it owns becomes `Draining` (a surviving node took its exact position, a single-hop move), so it KEEPS SERVING during the survivors' mounts. The overlap drain is therefore already the right machinery for a graceful leave; the missing piece is BOTH (a) a node-state that lets survivors re-own a position WITHOUT the leaving node disappearing, and (b) the leaving node's `Close()` waiting for that drain to finish.

**The gap today (the availability bug this subsection closes), AND why the obvious fix is self-contradictory.** On SIGTERM the run loop calls `Cluster.Close()`. The naive fix - "broadcast `memberlist.Leave()` but keep the transport up so survivors re-own the positions while the leaving node keeps serving forwards" - DOES NOT WORK, and that self-contradiction is the root cause of the measured ~58% in-process / ~70% staging drain availability. The leaving node is being asked to be BOTH gone (so survivors take over its ownership positions) AND alive (so it can keep serving the forwards during the drain), and a `memberlist.Leave()`-while-staying-alive cannot represent that. `memberlist.Leave()` broadcasts a "leaving" intent, but the node then KEEPS GOSSIPING AS ALIVE (the transport is up). Survivors receive the leave, briefly drop the node, then keep hearing it alive and RE-ADD it to their membership snapshot. Because ownership is derived from the live membership ring on EVERY node, the survivors that re-added the leaver compute it as STILL OWNING its positions, so they NEVER enter the overlap-`Acquiring` path for those positions: they see no predecessor, no handoff, they CLEAN-CUT acquire when (and only when) failure detection eventually evicts the leaver, and during the drain they keep routing writes at the leaver itself or refusing them. The instrumented failure: every survivor acquire decision during the drain saw the leaver still in the live ring, so almost none of the expected forwards fired, and writes bottomed out on the slow mount. `memberlist.Leave()` says "I am gone" while the transport says "I am alive"; membership cannot hold both, so the handoff never starts.

**The fix: a distinct DRAINING node-state - REACHABLE but YIELDING OWNERSHIP.** The node needs a state that is exactly what `memberlist.Leave()`-while-alive could not express: it stays a full, alive, addressable member (so survivors can keep forwarding to it AND it keeps serving), but it is EXCLUDED FROM OWNERSHIP on every node (so its positions redistribute to survivors, who then overlap-`Acquire` it as their predecessor). This is advertised by a gossiped per-member `Draining` bit, NOT by leaving the cluster. The sequence:

  - The leaving node SETS ITSELF DRAINING by flipping a `Draining` flag in its memberlist `Meta` and calling `memberlist.UpdateNode` so the flag gossips out. The node's `Meta` ALREADY carries its gRPC dial address (the address every forward resolves through); the `Draining` bit rides alongside it in the SAME `Meta` payload. The node stays ALIVE - it does NOT call `memberlist.Leave()` here.
  - Every node (including the draining node itself) DECODES the peer `Meta` on each gossip update and learns the `Draining` bit. The ring-from-membership reconcile EXCLUDES a `Draining` member from the OWNERSHIP ring: `LocateKeyN` / `desiredReplicaUnits` no longer assign it any replica position, so its positions redistribute to the non-draining members. Crucially, the draining member STAYS in the membership snapshot (alive, address known) - it is removed from OWNERSHIP, NOT from MEMBERSHIP. So the address remains resolvable for forwards even though no position is owned there any more.
  - The moment the draining `Meta` gossips, every survivor's reconcile recomputes ownership WITHOUT the draining node, sees its own index newly assigned to one of the draining node's old positions, identifies the draining node as the single-hop predecessor (it held that exact position in the prior-desired snapshot and no longer does in the live, draining-excluded set), enters `Acquiring`, and FORWARDS routed ops to the draining node's stored `PredecessorAddr` while it mounts. The draining node, still alive and serving, answers those position-addressed forwards from its `Draining` `mountMap[ru]` entries exactly as the loser side of any overlap drain.
  - The leaving node BLOCKS until every one of its positions has been handed off to a SERVING successor - i.e. until `drainCheck` has released them all (each released on observing the successor's serving marker at an epoch strictly above its own open epoch, the SAME release rule as a normal overlap drain). Only AFTER the drain completes (or the grace timeout fires) does the node do the REAL `memberlist.Leave()` + `Shutdown()` teardown.

So the order is: SET DRAINING (gossip the Meta bit) -> SERVE + WAIT (survivors acquire, the node keeps serving forwards, drainCheck releases each position) -> REAL `Leave()` + `Shutdown()`. The drain happens entirely while the node is a normal alive member; the actual departure happens only once there is nothing left to drain.

**This SUPERSEDES the ring-freeze stopgap.** An earlier attempt called `memberlist.Leave()` at the START of the drain (the self-contradiction above) and then tried to paper over the resulting snapshot collapse with a `leaving atomic.Bool` flag: once the node had broadcast `Leave()` it stopped gossiping, its own failure detector marked every peer dead within seconds, and its membership snapshot collapsed to empty - so the ring-from-membership reconcile would have rebuilt the ring from nothing and wiped the very survivors it needed to hand off to. The stopgap FROZE the ring (`leaving` made `reconcileRingFromMembership` a no-op) and removed only self from the ring, to preserve the survivors the leaver had captured at drain-start. The DRAINING node-state removes the need for all of it: because the node stays ALIVE (a real member) throughout the drain, its snapshot does NOT collapse, `reconcileRingFromMembership` works normally (it simply excludes `Draining` members from ownership), and there is no frozen ring to maintain. The `leaving` flag, its early-return guard in `reconcileRingFromMembership`, and the remove-self-only step are all REMOVED, replaced by the draining-`Meta`-driven ownership exclusion. (Keep any one of them only if it is proven still necessary after the draining-state lands.)

**Two memberlist verbs, split (Leave vs Shutdown), used ONLY at the END.** memberlist exposes `Leave()` (broadcast the graceful departure so peers record a clean leave and stop re-adding the node) DISTINCT from `Shutdown()` (tear down the local transport). The membership wrapper's `Close()` today does BOTH at once. They are still split - but the split now serves a different purpose than the earlier draft assumed: the REAL `Leave()` is deferred to AFTER the drain, not used to START it. Starting the drain with `Leave()` is exactly the self-contradiction above (leave-intent + still-alive => survivors re-add). The drain is started by the `Draining` `Meta` bit (the node stays fully alive). Only once the drain is done does the node call the real `Leave()` (now correct: the node really is going away, so survivors recording a clean leave is right) followed by `Shutdown()` of the transport in the existing teardown.

**The drain entry point.** A new `Cluster` method `DrainForLeave(ctx)` (equivalently `GracefulLeave(timeout)`): SET THIS NODE DRAINING via the membership `Meta` flag (so every node excludes it from ownership, survivors re-own its positions and start forwarding to it, while it stays alive and serving), then BLOCK until the `handoffPhase` map holds no `Draining` entry owned by this node (every position `drainCheck` released, `mountMap` carries no position this node still owns) OR `ctx` is cancelled / the timeout fires. The reconcile loop, the serving path, `drainCheck`, and the forward path all stay running during this wait. AFTER the wait returns (drained or timed out), the existing teardown - the real membership `Leave()` then `Shutdown()` - runs. The shutdown path calls `DrainForLeave` at the TOP of `Close()` - gated on config, BEFORE any teardown, while the loops are still running - and then proceeds with the existing teardown (now including the real `Leave()`) unchanged. Putting the drain at the top of `Close()` keeps it self-contained: the SIGTERM handler in the run loop just calls `Close()` as it does today, so no run-loop change is required.

**Config gate.** A `Cluster` config field `GracefulLeaveDrainTimeout time.Duration` controls it. `0` = DISABLED = exactly today's behavior (`Close()` does not drain; the gap remains - this is also the break-demo state for the acceptance gate). When `> 0` AND the cluster is multi-backend overlap (`multiReplicated()`), `Close()` calls `DrainForLeave(GracefulLeaveDrainTimeout)` first. The R=1 lease-handoff path and the legacy per-node path are UNTOUCHED (the field is a no-op outside multi-backend overlap, consistent with the rest of Phase 2e living behind `multiReplicated()`).

**What goes Draining vs the residual.** A graceful leave maximizes `Draining` handoff because a position the leaving node owns is, by definition, taken over by a SURVIVING node - that is a single-hop move (the leaving node held position rP of unit K; a survivor now holds rP of K), which the overlap machine resolves to `Draining` on the leaving side and `Acquiring` (with the leaving node as the identified predecessor) on the survivor side. So in the common case EVERY position the leaving node serves goes `Draining` and is covered by the drain wait. The residual (NOT covered by this subsection, degrading to a gap or to the old behavior):

  - A position that hits the PLAIN clean-cut RELEASE path (overlap-move vs plain-release, above) rather than `Draining`: this happens when the leaving node simply DROPS OUT of a unit's replica set and some OTHER already-mounted replica covers W with no successor taking the leaving node's EXACT position. There is no successor to forward to and nobody routes to the leaving node after the ring rotation, so there is nothing to drain - the eager release is correct and the position stays available via the surviving replicas. This is NOT a gap; it is simply outside the drain machinery. (A position whose successor is AMBIGUOUS / multi-hop - the single-hop fallback - would degrade to `errUnitAcquiring` + Option-A retry on the survivor side, and on the leaving side stay `Draining` until the timeout; see the timeout residual next.)
  - A position whose successor is STUCK (its `Acquiring` mount never completes within the grace budget): the drain wait times out, `Close()` proceeds, and that one position is UNSERVED from teardown until the survivor finally mounts - exactly today's gap, for that position only. This is no worse than the disabled (timeout 0) behavior, and it is bounded by the operator's grace budget rather than unbounded.

**The invariant.** For a position with a REACHABLE, single-hop successor, a graceful leave has NO unserved window: the leaving node keeps serving (directly and via the survivor's forward) until the survivor is `Ready` and `drainCheck` releases the position, and only then does the leaving node close. A continuous writer through a graceful one-node leave therefore sees ~100% ack and zero unserved window (vs the gap today). The residual: a position whose successor is STUCK degrades to the old gap AFTER the timeout, and a position hitting the plain clean-cut RELEASE path is not covered by the drain (it does not need to be - the surviving replicas cover it). No-acked-write-lost and single-writer-fence are unchanged (the drain reuses the exact overlap release rule; the leaving node never releases a position before its successor is provably serving).
...
- [~] **v0.8** (in progress) - per-shard lease-handoff storage: copy-free rebalance + online doubling resharding. Replaces the per-node single-database model (one slatedb per node, rebalance copies keys) with a FIXED power-of-two count N of self-contained storage-unit databases whose OWNERSHIP is a lease (the ring is re-keyed onto unit ids; handoff is close-on-old-owner / open-at-epoch+1-on-new-owner, zero bytes copied). N is bounded by per-engine memtable memory; growth by doubling makes resharding an online per-unit bisect with no global re-partition. Full design in the "Per-shard lease-handoff storage" section above. Sub-tasks:
  ...
  - [ ] Overlap handoff (Phase 2e, Option B): the OLD owner of a moving `ReplicaUnit` KEEPS SERVING until the NEW owner has fully acquired (mounted + durably fenced + ready), and during the overlap the NEW owner FORWARDS routed ops to the predecessor via a POSITION-ADDRESSED forward (the explicit `ReplicaUnit` rides on the wire so the predecessor resolves `mountMap[ru]` directly and serves the `Draining` entry - a real proto change, NOT a key-only forward) so the ring rotation does not strand the position; the predecessor is identified from a retained prior-desired-replica-set snapshot (the multi analogue of legacy `lastEvalRing`), and overlap is SINGLE-HOP (an ambiguous/multi-hop predecessor degrades to Option A); the mount flip happens atomically at the new owner's `mountMap[ru]` insert; the old owner releases exactly once when the new owner is READY, detected POLL-ONLY via a durable, poll-observable SERVING MARKER the new owner writes once after the flip (the durable manifest FENCE epoch, which bumps at open-START, is strictly weaker and NEVER releases on its own - no `ReplicaHandoffReady` push RPC). The marker read is POINT-IN-TIME: a NEW crash in the read-to-release gap leaves the position unserved until the next reconcile, no acked-write loss. Availability no longer depends on mount time (a 30s mount is a non-event). Expressed as ONE per-`ReplicaUnit` ownership-transition state machine (`Acquiring/Ready` on the gainer, `Draining/Releasing` on the loser, over the `Owned/Absent` poles) replacing the position-change branch of `reconcileReplicaUnits`, with `mountMap` / phase map / `HandoffState` re-keyed by `ReplicaUnit` and a `priorDesiredReplicas` snapshot added; pure transition types in `pkg/storageunit` (`handoff.go`), the controller in `pkg/cluster` (`multibackend_overlap.go`), `DurableEpochReplica` + `WriteServingMarker` / `ReadServingMarker` on the `ReplicaBackendFactory` interface. Open implementation-must-verify: the new owner's open must replay the full durable WAL tail AND the manifest-epoch fence must be effective no later than the WAL-recovery cutoff (pin with a write-then-fence test AND a write-through-forward-concurrent-with-flip test; slatedb-go is an opaque FFI binding, and the slate `OpenReplicaUnit` fence-then-recover order may need pinning). Acceptance: an in-process loss-oracle gate with an ARBITRARILY SLOW `OpenReplicaUnit` asserts zero acked loss AND acked/attempted ~100% even under a multi-second mount (where Option A alone collapses), demonstrated to FAIL on a forced clean-cut with Option-A retry also off. Option A's retry is retained as the safety net for the residual unserved instant (predecessor unreachable OR ambiguous/multi-hop, crash mid-handoff, pure-new-mount convergence); A and B compose. See "v0.8 Phase 2e" above.
  - [ ] Graceful leave / DRAINING node-state (Phase 2e, scale-down): a leaving node enters a distinct DRAINING state - REACHABLE but YIELDING OWNERSHIP - advertised via a gossiped per-member `Draining` bit in the memberlist `Meta` (alongside the gRPC address), set by `membership.SetDraining(true)` which calls `memberlist.UpdateNode` (the node STAYS alive, does NOT `Leave()`). Every node decodes the bit and `reconcileRingFromMembership` EXCLUDES a `Draining` member from the consistent-hash OWNERSHIP ring (its positions redistribute to survivors via `LocateKeyN`) while it stays in MEMBERSHIP (address resolvable for forwards). Survivors thus enter overlap-`Acquiring` with the draining node as the single-hop predecessor and forward to its stored `PredecessorAddr` while it keeps serving; `DrainForLeave(ctx)` blocks until `ownedPositionCount() == 0` (all positions handed off + released by `drainCheck`) or the timeout fires, THEN the real `membership.Leave()` + `Shutdown()` teardown runs. Order: SET DRAINING -> SERVE + WAIT -> REAL LEAVE + SHUTDOWN. This FIXES the root-cause design flaw where `memberlist.Leave()`-while-staying-alive was self-contradictory (leave-intent + still-gossiping-alive => survivors RE-ADD the leaver to the live ring and never enter the handoff path, so the drain delivered ~58% in-process / ~70% staging availability). SUPERSEDES the ring-freeze stopgap (the `leaving atomic.Bool` + the `reconcileRingFromMembership` early-return + the `ring.Remove(self)`), which papered over the snapshot collapse that the early `Leave()` itself caused: the draining node stays alive so its snapshot never collapses. Gated by `GracefulLeaveDrainTimeout` (`0` = disabled = the break-demo state); behind `multiReplicated()`. Acceptance: a continuous writer through a graceful one-node leave asserts ~100% ack + zero unserved window (vs the gap with timeout 0), reusing the slow-`OpenReplicaUnit` injection. See "v0.8 Phase 2e: Graceful leave (scale-down)" above.
